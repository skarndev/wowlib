...
namespace wowlib::db {
  ...
  public:
    ...
    virtual void set_int(std::size_t record, std::size_t column, std::size_t element, std::int64_t value) = 0;
    virtual void set_float(std::size_t record, std::size_t column, std::size_t element, float value) = 0;
    virtual void set_string(std::size_t record, std::size_t column, std::size_t element, std::string_view value) = 0;
  ...
  public:
    ...
    virtual std::size_t size() const = 0;
    virtual std::uint32_t id_of(std::size_t record) const = 0;
    virtual std::int64_t get_int(std::size_t record, std::size_t column, std::size_t element) const = 0;
    virtual std::uint32_t get_slot(std::size_t record, std::size_t column, std::size_t element) const = 0;
    virtual std::string_view get_string(std::size_t record, std::size_t column, std::size_t element) const = 0;
  };
  ...
  namespace detail {
    ...
    inline std::size_t record_stride(std::span<const Column> schema) {
      std::size_t bytes = 0;
      for (const Column& col : schema) bytes += col.inline_bytes();
      return bytes;
    }
    ...
    inline std::uint32_t field_slot_count(std::span<const Column> schema) {
      ...
      for (const Column& col : schema) slots += col.field_slots();
      return slots;
    }
    ...
    inline std::size_t string_slot_count(std::span<const Column> schema) {
      std::size_t slots = 0;
      for (const Column& col : schema) slots += col.string_slots();
      return slots;
    }
    ...
    inline bool id_is_noninline(std::span<const Column> schema) {
      for (const Column& col : schema)
        if (col.is_id) return col.noninline;
      return false;
    }
