...
namespace wowlib::db
{
  Result<void> read_wdbc(const TableInfo& info, std::span<const std::byte> data,
                         RecordSink& sink, TableState& state)
  {
    if (data.size() < sizeof(WdbcHeader))
      return make_error(ErrorCode::TableTruncated,
                        std::format("{}: {} bytes is too small for a WDBC header", info.name,
                                    data.size()));
    WdbcHeader header;
  ...
  Result<FileBuffer> write_wdbc(const TableInfo& info, const RecordSource& source,
                                const TableState& state)
  {
    ...
    header.field_count =
      state.source_magic != 0 ? state.field_count : detail::field_slot_count(info.schema);
    header.record_size =
      state.source_magic != 0 ? state.record_size : static_cast<std::uint32_t>(stride);
