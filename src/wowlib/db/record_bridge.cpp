...
namespace wowlib::db {
  namespace {
    ...
    const std::byte* elem_ptr(const std::byte* rec, const ColumnAccess& a, std::size_t element) {
      return rec + a.offset + element * a.elem_stride;
    }
    ...
    void store_int(std::byte* rec, const ColumnAccess& a, std::size_t element, std::int64_t v) {
      switch (a.kind) {
      case AccessKind::I8: {
    ...
    std::int64_t load_int(const std::byte* rec, const ColumnAccess& a, std::size_t element) {
    ...
    const std::string& string_at(const std::byte* rec, const ColumnAccess& a, std::size_t element) {
    ...
    std::uint32_t id_of_record(const RecordOps& ops, const std::byte* rec) {
      if (ops.id_column == SIZE_MAX) return 0;
      return static_cast<std::uint32_t>(load_int(rec, ops.columns[ops.id_column], 0));
    }
  }
  ...
  void ErasedRecordSink::set_int(std::size_t record, std::size_t column, std::size_t element, std::int64_t value) {
    store_int(ops_->at(vec_, record), ops_->columns[column], element, value);
  }

  void ErasedRecordSink::set_float(std::size_t record, std::size_t column, std::size_t element, float value) {
  ...
  void ErasedRecordSink::set_string(std::size_t record,
                                    std::size_t column,
                                    std::size_t element,
                                    std::string_view value) {
    const ColumnAccess& a = ops_->columns[column];
    if (a.kind == AccessKind::Str || a.kind == AccessKind::LocStr) string_at(ops_->at(vec_, record), a, element) =
      std::string{value};
  }
  ...
  void ErasedRecordSink::clone_with_id(std::size_t src, std::uint32_t new_id) {
    ...
    if (ops_->id_column != SIZE_MAX)
      store_int(ops_->at(vec_, ops_->size(vec_) - 1), ops_->columns[ops_->id_column], 0, new_id);
  }

  std::size_t ErasedRecordSink::find_by_id(std::uint32_t record_id) const {
    const std::size_t n = ops_->size(vec_);
    for (std::size_t i = 0; i < n; ++i)
      if (id_of_record(*ops_, ops_->cat(vec_, i)) == record_id) return i;
    return n;
  }
  ...
  std::int64_t ErasedRecordSource::get_int(std::size_t record, std::size_t column, std::size_t element) const {
    return load_int(ops_->cat(vec_, record), ops_->columns[column], element);
  }

  std::uint32_t ErasedRecordSource::get_slot(std::size_t record, std::size_t column, std::size_t element) const {
    const ColumnAccess& a = ops_->columns[column];
  ...
  std::string_view ErasedRecordSource::get_string(std::size_t record, std::size_t column, std::size_t element) const {
    const ColumnAccess& a = ops_->columns[column];
