...
namespace wowlib::db
{
  namespace
  {
    ...
    {
      if (build < Build{2, 1, 0, 6692})
        return 8;
      if (build < Build{4, 0, 0, 0})
        return 16;
      return std::nullopt;
    }
    ...
    std::string snake(std::string_view name)
    {
      ...
      {
        if (i > 0 && upper(name[i]) && i + 1 < name.size() &&
            lower(name[i + 1]))
          pass1 += '_';
        pass1 += name[i];
      }
      ...
      {
        if (i > 0 && upper(pass1[i]) &&
            (lower(pass1[i - 1]) || digit(pass1[i - 1])))
          pass2 += '_';
        pass2 += pass1[i];
      }
    ...
    {
      if (const auto at = line.find("//"); at != std::string_view::npos)
    ...
    bool parse_column_line(std::string_view body, Definition& def)
    {
      ...
      if (name.empty() || !std::ranges::all_of(name, ident_char))
        return false;
    ...
    std::optional<Entry> parse_entry(std::string_view body)
    {
      ...
      if (body.starts_with('$'))
      {
        ...
        while (!anns.empty())
        {
          ...
          if (ann == "id")
            entry.is_id = true;
          else if (ann == "relation")
            entry.is_relation = true;
          else if (ann == "noninline")
            entry.noninline = true;
      ...
      std::size_t at = 0;
      while (at < body.size() && ident_char(body[at]))
        ++at;
      if (at == 0)
        return std::nullopt;
      ...
      if (body.starts_with('<'))
      {
        ...
        int bits = 0;
        auto [next, ec] =
          std::from_chars(body.data(), body.data() + body.size(), bits);
        if (ec != std::errc{} || next == body.data() || *next != '>')
          return std::nullopt;
        entry.bits = bits;
    ...
    std::optional<Definition> parse_dbd(const std::string& text)
    {
      ...
      if (!current.empty())
        paragraphs.push_back(std::move(current));
      if (paragraphs.empty() || paragraphs.front().front() != "COLUMNS")
        return std::nullopt;
      ...
      for (std::size_t p = 1; p < paragraphs.size(); ++p)
      {
        ...
        if (block.builds.empty() && !block.has_layouts)
          return std::nullopt;
    ...
    struct Member
    {
      ...
      bool same_shape(const Member& other) const
      {
        return name == other.name && type == other.type &&
               ...
               locale_count == other.locale_count && is_id == other.is_id &&
               is_relation == other.is_relation &&
               noninline == other.noninline;
      }
    };

    /** build_members for one target; nullopt = the table skips this era
        (dbdgen warns and drops the whole table for that target). */
    std::optional<std::vector<Member>> members_for(const Definition& def,
                                                   const VersionBlock& block,
                                                   Build build)
    {
      ...
      if (std::set<std::string_view>(names.begin(), names.end()).size() !=
          names.size())
        return std::nullopt; // member name collision
      std::vector<Member> members;
      for (std::size_t i = 0; i < block.entries.size(); ++i)
      {
        const Entry& entry = block.entries[i];
        const auto decl = def.columns.find(entry.name);
        if (decl == def.columns.end())
          return std::nullopt; // no COLUMNS declaration
        const std::string& type = decl->second.type;

        Member m{};
        m.name = names[i];
        m.array_len =
          static_cast<std::uint16_t>(entry.array_len.value_or(1));
        m.is_id = entry.is_id;
        ...
        {
          if (type != "int")
            return std::nullopt; // sized entry on a non-int column
          if (*entry.bits != 8 && *entry.bits != 16 && *entry.bits != 32 &&
              *entry.bits != 64)
            return std::nullopt; // unsupported width
        ...
        {
          if (entry.array_len)
            return std::nullopt; // locstring arrays unsupported
          if (langs)
          {
    ...
    {
    public:
      void add_table(const std::string& name, const std::string& disk,
                     const std::vector<Range>& ranges)
      {
        ...
        for (const Range& range : ranges)
        {
          ...
          for (const Member& m : range.members)
          {
            ...
            columns_.push_back(static_cast<unsigned char>(
              (m.is_signed ? 1 : 0) | (m.is_id ? 2 : 0) |
              (m.is_relation ? 4 : 0) | (m.noninline ? 8 : 0)));
    ...
    private:
      ...
      {
        const auto [at, fresh] =
          interned_.try_emplace(text, static_cast<std::uint32_t>(pool_.size()));
        if (fresh)
        {
  ...
  Result<SchemaCatalog> SchemaCatalog::from_dbd_dir(
    const std::filesystem::path& definitions)
  {
    ...
    if (ec)
      return make_error(ErrorCode::IoError,
                        "cannot read WoWDBDefs definitions directory: " +
                          definitions.string());
    ...
    {
      if (!entry.is_regular_file() || entry.path().extension() != ".dbd")
        continue;
      ...
      const bool identifier =
        !table.empty() && !(table.front() >= '0' && table.front() <= '9') &&
        std::ranges::all_of(table, ident_char);
      if (!identifier)
        continue;
    ...
    for (const auto& [table, path] : files)
    {
      ...
      if (!def)
        continue; // dbdgen warns and skips; the loader skips silently
      ...
      for (std::size_t era = 0; era < targets.size(); ++era)
      {
        ...
        const auto same = [&](const Range& range) {
          return range.members.size() == members->size() &&
                 std::ranges::equal(range.members, *members,
                                    [](const Member& a, const Member& b) {
                                      return a.same_shape(b);
                                    });
        };
        if (!ranges.empty() && same(ranges.back()))
          ranges.back().era_mask |=
            static_cast<std::uint16_t>(1u << era);
        else
          ranges.push_back(Range{static_cast<std::uint16_t>(1u << era),
                                 std::move(*members)});
      }
      if (ranges.empty())
        continue;
      const std::string disk =
        table == "ItemSparseLegacy" ? "Item-sparse" : table;
    ...
    if (!view.valid() || view.table_count() == 0)
      return make_error(ErrorCode::SchemaBlobInvalid,
                        "no WoWDBDefs definition in " + definitions.string() +
