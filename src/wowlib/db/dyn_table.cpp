...
namespace wowlib::db
{
  namespace detail
  {
    ...
    ColumnRows::ColumnRows(std::span<const Column> schema) : schema_{schema}
    {
      ...
      for (std::size_t c = 0; c < schema.size(); ++c)
      {
        ...
        facts_.push_back(f);
        if (col.is_id && id_column_ == SIZE_MAX)
          id_column_ = c;
      }
    }

    std::byte* ColumnRows::pod_ptr(std::size_t record, std::size_t column,
                                   std::size_t element)
    {
      const Facts& f = facts_[column];
      return cols_[column].pod.data() +
             (record * f.pod_slots + element) * f.elem_bytes;
    }

    const std::byte* ColumnRows::pod_ptr(std::size_t record, std::size_t column,
    ...
    void ColumnRows::reserve(std::size_t n)
    {
      for (std::size_t c = 0; c < cols_.size(); ++c)
      {
        ...
        if (f.pod_slots != 0)
          cols_[c].pod.reserve(n * f.pod_slots * f.elem_bytes);
        if (f.str_slots != 0)
          cols_[c].strs.reserve(n * f.str_slots);
        if (f.has_flags)
          cols_[c].flags.reserve(n);
      }
    }
    ...
    std::size_t ColumnRows::add()
    {
      for (std::size_t c = 0; c < cols_.size(); ++c)
      {
        const Facts& f = facts_[c];
        if (f.pod_slots != 0)
          cols_[c].pod.resize(cols_[c].pod.size() +
                                  std::size_t{f.pod_slots} * f.elem_bytes,
                              std::byte{0});
        if (f.str_slots != 0)
          cols_[c].strs.resize(cols_[c].strs.size() + f.str_slots);
        if (f.has_flags)
          cols_[c].flags.push_back(0);
      }
      return rows_++;
    }
    ...
    std::uint32_t ColumnRows::id_of(std::size_t record) const
    {
      ...
      return static_cast<std::uint32_t>(
          load_pod_int(pod_ptr(record, id_column_, 0), facts_[id_column_].kind));
    }

    void ColumnRows::set_int(std::size_t record, std::size_t column,
                             std::size_t element, std::int64_t value)
    {
      ...
      if (f.pod_slots != 0 && f.kind != AccessKind::F32)
        store_pod_int(pod_ptr(record, column, element), f.kind, value);
      // float / string columns never receive set_int (record_bridge parity).
    }

    void ColumnRows::set_float(std::size_t record, std::size_t column,
                               std::size_t element, float value)
    {
      if (facts_[column].kind == AccessKind::F32)
        std::memcpy(pod_ptr(record, column, element), &value, sizeof value);
    }

    void ColumnRows::set_string(std::size_t record, std::size_t column,
                                std::size_t element, std::string_view value)
    {
      const Facts& f = facts_[column];
      if (f.str_slots != 0)
        cols_[column].strs[record * f.str_slots + element] = std::string{value};
    }
    ...
    void ColumnRows::clone_with_id(std::size_t src, std::uint32_t new_id)
    {
      ...
      for (std::size_t c = 0; c < cols_.size(); ++c)
      {
        ...
        if (f.pod_slots != 0)
        {
          ...
          std::memcpy(s.pod.data() + fresh * stride, s.pod.data() + src * stride,
                      stride);
        }
        for (std::size_t e = 0; e < f.str_slots; ++e)
          s.strs[fresh * f.str_slots + e] = s.strs[src * f.str_slots + e];
        if (f.has_flags)
          s.flags[fresh] = s.flags[src];
      }
      if (id_column_ != SIZE_MAX)
        store_pod_int(pod_ptr(fresh, id_column_, 0), facts_[id_column_].kind,
                      new_id);
    }
    ...
    std::int64_t ColumnRows::get_int(std::size_t record, std::size_t column,
                                     std::size_t element) const
    {
      const Facts& f = facts_[column];
      if (f.kind == AccessKind::LocStr)
      ...
      if (f.pod_slots == 0 || f.kind == AccessKind::F32)
        return 0; // as the row-store bridge answered for non-int members
      return load_pod_int(pod_ptr(record, column, element), f.kind);
    }

    std::uint32_t ColumnRows::get_slot(std::size_t record, std::size_t column,
                                       std::size_t element) const
    {
      const Facts& f = facts_[column];
      switch (f.kind)
      {
        ...
        default:
          return static_cast<std::uint32_t>(
              load_pod_int(pod_ptr(record, column, element), f.kind));
      }
    }

    std::string_view ColumnRows::get_string(std::size_t record,
                                            std::size_t column,
                                            std::size_t element) const
    {
      const Facts& f = facts_[column];
      ...
      for (std::size_t c = 0; c < cols_.size(); ++c)
      {
        ...
        if (f.pod_slots != 0)
        {
          ...
          s.pod.erase(s.pod.begin() + static_cast<std::ptrdiff_t>(record * stride),
                      s.pod.begin() +
                          static_cast<std::ptrdiff_t>((record + 1) * stride));
        }
        if (f.str_slots != 0)
          s.strs.erase(
              s.strs.begin() + static_cast<std::ptrdiff_t>(record * f.str_slots),
              s.strs.begin() +
                  static_cast<std::ptrdiff_t>((record + 1) * f.str_slots));
        if (f.has_flags)
          s.flags.erase(s.flags.begin() + static_cast<std::ptrdiff_t>(record));
      }
      --rows_;
    }

    std::string& ColumnRows::string_slot(std::size_t record, std::size_t column,
                                         std::size_t element)
    {
    ...
    std::uint32_t ColumnRows::flags_slot(std::size_t record,
  ...
  Result<DynTable> DynTable::open(std::string_view table, ClientVersion version)
  {
    return SchemaCatalog::embedded().lookup(table, version).transform(
        [&](const TableSchema& schema) { return from_schema(schema, version); });
  }
#endif
  ...
  DynTable::DynTable(const DynTable& other)
    : TableBase{other}, rows_{other.rows_}, version_{other.version_},
      name_{other.name_}, disk_name_{other.disk_name_}
  {
    wire_();
  }
  ...
  DynTable::DynTable(DynTable&& other) noexcept
    : TableBase{std::move(other)}, rows_{std::move(other.rows_)},
      version_{other.version_}, name_{other.name_}, disk_name_{other.disk_name_}
  {
    wire_();
  }
  ...
  Result<std::size_t> DynTable::column_index(std::string_view name) const
  {
    const std::span<const Column> schema = rows_.schema();
    for (std::size_t c = 0; c < schema.size(); ++c)
      if (schema[c].name_view() == name)
        return c;
    return make_error(ErrorCode::TableUnknown,
                      std::format("{}: no column named '{}'", name_, name));
  }
  ...
  Result<void> DynTable::erase_row(std::size_t row)
  {
    if (row >= rows_.size())
      return make_error(ErrorCode::OffsetOutOfBounds,
                        std::format("{}: row {} out of range ({} rows)", name_,
                                    row, rows_.size()));
    rows_.erase_row(row);
    return {};
  }
  ...
  Result<const detail::ColumnRows::Facts*> DynTable::check_cell(
      ...
      ColumnType expected) const
  {
    ...
    if (row >= rows_.size() || column >= schema.size())
      return make_error(ErrorCode::OffsetOutOfBounds,
                        std::format("{}: cell ({}, {}) out of range ({} rows, "
                                    "{} columns)",
                                    name_, row, column, rows_.size(),
                                    schema.size()));
    const Column& col = schema[column];
    if (col.type != expected)
      return make_error(
          ErrorCode::SchemaMismatch,
          std::format("{}.{}: the column holds {}, not {}", name_,
                      col.name_view(), enum_name(col.type), enum_name(expected)));
    const detail::ColumnRows::Facts& f = rows_.facts(column);
    const std::size_t slots =
        f.pod_slots != 0 ? f.pod_slots : std::size_t{f.str_slots};
    if (element >= slots)
      return make_error(ErrorCode::OffsetOutOfBounds,
                        std::format("{}.{}: element {} out of range ({} slots)",
                                    name_, col.name_view(), element, slots));
    return &f;
  }

  Result<std::int64_t> DynTable::get_int(std::size_t row, std::size_t column,
                                         std::size_t element) const
  {
  ...
  Result<void> DynTable::set_int(std::size_t row, std::size_t column,
                                 std::int64_t value, std::size_t element)
  {
    return check_cell(row, column, element, ColumnType::Int)
        .transform([&](auto) { rows_.set_int(row, column, element, value); });
  }

  Result<float> DynTable::get_float(std::size_t row, std::size_t column,
                                    std::size_t element) const
  {
    return check_cell(row, column, element, ColumnType::Float).transform([&](auto) {
      float out;
      std::memcpy(&out, rows_.pod_bytes(column).data() +
                            (row * rows_.facts(column).pod_slots + element) * 4,
                  sizeof out);
      return out;
    });
  }

  Result<void> DynTable::set_float(std::size_t row, std::size_t column,
                                   float value, std::size_t element)
  {
    return check_cell(row, column, element, ColumnType::Float)
        .transform([&](auto) { rows_.set_float(row, column, element, value); });
  }

  Result<std::string_view> DynTable::get_string(std::size_t row,
                                                std::size_t column,
                                                std::size_t element) const
  {
    ...
    return check_cell(row, column, element, ColumnType::LocString)
        .transform([&](auto) { return rows_.get_string(row, column, element); });
  }

  Result<void> DynTable::set_string(std::size_t row, std::size_t column,
                                    std::string_view value, std::size_t element)
  {
    const auto str = check_cell(row, column, element, ColumnType::String);
    if (str)
    {
    ...
    return check_cell(row, column, element, ColumnType::LocString)
        .transform([&](auto) { rows_.set_string(row, column, element, value); });
  }

  Result<std::uint32_t> DynTable::locstring_flags(std::size_t row,
                                                  std::size_t column) const
  {
    return check_cell(row, column, 0, ColumnType::LocString)
        .transform([&](auto) { return rows_.flags_slot(row, column); });
  }

  Result<void> DynTable::set_locstring_flags(std::size_t row, std::size_t column,
                                             std::uint32_t flags)
  {
    return check_cell(row, column, 0, ColumnType::LocString)
  ...
  Result<PodColumnView> DynTable::pod_column(std::size_t column) const
  {
    ...
    if (col.type != ColumnType::Int && col.type != ColumnType::Float)
      return make_error(ErrorCode::SchemaMismatch,
                        std::format("{}.{}: only numeric columns have a "
                                    "zero-copy view",
                                    name_, col.name_view()));
    const detail::ColumnRows::Facts& f = rows_.facts(column);
    PodColumnView out{};
