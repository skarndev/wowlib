...
namespace wowlib::db
{
  SchemaCatalog SchemaCatalog::materialize(std::span<const unsigned char> bytes,
                                           std::vector<unsigned char> owned)
  {
    ...
    for (std::size_t i = 0; i < view.era_count(); ++i)
      out.eras_.push_back(view.era(i));
    ...
    for (std::size_t i = 0; i < view.range_count(); ++i)
    {
      ...
      out.ranges_.push_back(RangeIndex{entry.first_column, entry.column_count,
                                       entry.era_mask});
    }
    ...
    for (std::size_t i = 0; i < view.table_count(); ++i)
    {
      ...
      out.tables_.push_back(TableIndex{view.string_at(entry.name_off),
                                       view.string_at(entry.disk_name_off),
                                       entry.first_range, entry.range_count});
    }
    return out;
  }
  ...
  Result<SchemaCatalog> SchemaCatalog::from_blob(std::vector<unsigned char> bytes)
  {
    ...
    if (!view.valid())
      return make_error(ErrorCode::SchemaBlobInvalid,
                        "not a WDBS v1 schema blob (bad magic, version or "
                        "section bounds)");
  ...
  Result<SchemaCatalog> SchemaCatalog::from_blob_file(const std::filesystem::path& path)
  {
    ...
    if (!in)
      return make_error(ErrorCode::IoError,
                        "cannot open schema blob: " + path.string());
  ...
  Result<std::size_t> SchemaCatalog::era_index_of(ClientVersion version) const
  {
    for (std::size_t i = 0; i < eras_.size(); ++i)
      if (eras_[i].major == version.major)
        return i;
    return make_error(ErrorCode::UnsupportedClientVersion,
                      "no targeted era shares major version " +
                          std::to_string(version.major));
  }

  Result<TableSchema> SchemaCatalog::lookup(std::string_view table,
                                            ClientVersion version) const
  {
    ...
    if (found == nullptr)
      return make_error(ErrorCode::TableUnknown,
                        "schema catalog knows no table '" + std::string{table} +
                            '\'');
    return era_index_of(version).and_then(
        [&](std::size_t era) -> Result<TableSchema> {
          for (std::size_t r = 0; r < found->range_count; ++r)
          {
            const RangeIndex& range = ranges_[found->first_range + r];
            if ((range.era_mask & (std::uint16_t{1} << era)) == 0)
              continue;
            return TableSchema{
                found->name, found->disk_name,
                std::span<const Column>{columns_}.subspan(range.first_column,
                                                          range.column_count)};
          }
          return make_error(ErrorCode::UnsupportedClientVersion,
                            "table '" + std::string{found->name} +
                                "' has no schema for the era of build " +
  ...
  #embed <wowlib_schema.wdbs>
    ...
    static_assert(blob::View{std::span<const unsigned char>{
                      embedded_schema_blob}}.valid(),
  ...
  {
    static const SchemaCatalog catalog = materialize(
        std::span<const unsigned char>{detail::embedded_schema_blob}, {});
    return catalog;
  }
#endif
}
