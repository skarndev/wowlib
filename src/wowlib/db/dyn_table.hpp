...
namespace wowlib::db {
  namespace detail {
    ...
    public:
      ...
      std::string& string_slot(std::size_t record, std::size_t column, std::size_t element);
    ...
    private:
      ...
      std::byte* pod_ptr(std::size_t record, std::size_t column, std::size_t element);
      const std::byte* pod_ptr(std::size_t record, std::size_t column, std::size_t element) const;
  ...
  public:
    ...
    Result<Column> column_info(std::size_t column [[=welder::doc("the column index")]]) const;
    ...
    Result<std::size_t> column_index(std::string_view name [[=welder::doc("the column name, case-sensitive")]]) const;
    ...
    Result<std::size_t> find_by_id(std::uint32_t id [[=welder::doc("the primary key value")]]) const;
    ...
    [[=welder::doc("Read an integer cell, sign-extended from the column's "
      ...
      =welder::returns("the value; raises on a non-Int column or an "
        "out-of-range index")]]
    Result<std::int64_t> get_int(std::size_t row [[=welder::doc("the row index")]],
    ...
    [[=welder::doc("Write an integer cell (truncates to the column's exact "
      ...
      =welder::returns("nothing; raises on a non-Int column or an "
        "out-of-range index")]]
    Result<void> set_int(std::size_t row [[=welder::doc("the row index")]],
                         std::size_t column [[=welder::doc("an Int column's index")]],
    ...
    [[=welder::doc("Read a float cell."),
      =welder::returns("the value; raises on a non-Float column or an "
        "out-of-range index")]]
    Result<float> get_float(std::size_t row [[=welder::doc("the row index")]],
    ...
    [[=welder::doc("Write a float cell."),
      =welder::returns("nothing; raises on a non-Float column or an "
        "out-of-range index")]]
    Result<void> set_float(std::size_t row [[=welder::doc("the row index")]],
                           std::size_t column [[=welder::doc("a Float column's index")]],
    ...
    [[=welder::doc("Read a string cell (String columns: the array element; "
      ...
      =welder::returns("the text; raises on a non-string column or an "
        "out-of-range index")]]
    Result<std::string_view> get_string(std::size_t row [[=welder::doc("the row index")]],
    ...
    [[=welder::doc("Write a string cell (String columns: the array element; "
      ...
      =welder::returns("nothing; raises on a non-string column or an "
        "out-of-range index")]]
    Result<void> set_string(std::size_t row [[=welder::doc("the row index")]],
                            std::size_t column [[=welder::doc("a String/LocString column's index")]],
    ...
    Result<std::uint32_t> locstring_flags(std::size_t row [[=welder::doc("the row index")]],
    ...
    Result<void> set_locstring_flags(std::size_t row [[=welder::doc("the row index")]],
  ...
  private:
    ...
    void wire_() {
      core_.wire(&rows_, &rows_, TableInfo{version_, disk_name_, rows_.schema()});
    }

    /** Shared bounds/kind gate for the cell accessors. */
    Result<const detail::ColumnRows::Facts*> check_cell(std::size_t row,
                                                        std::size_t column,
                                                        std::size_t element,
