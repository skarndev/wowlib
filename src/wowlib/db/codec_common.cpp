...
namespace wowlib::db::detail
{
  ...
  void write_int(FileBuffer& out, std::int64_t value, std::size_t bytes)
  {
    ...
    for (std::size_t i = 0; i < bytes; ++i)
      out.push_back(static_cast<std::byte>((raw >> (i * 8)) & 0xff));
  }
  ...
  std::uint32_t StringPool::resolve(std::string_view value,
                                    const std::vector<std::uint32_t>& journal, std::size_t& cursor)
  {
    ...
    if (slot < journal.size())
    {
      ...
      if (block.at(original) == owned)
        return original;
    }
    if (const auto it = lookup.find(owned); it != lookup.end())
      return it->second;
                            ...
                            const TableState& state, std::size_t& cursor)
  {
    ...
    for (const Column& c : schema)
    {
      ...
      switch (c.type)
      {
        case ColumnType::Int:
          for (std::uint16_t e = 0; e < c.array_len; ++e)
            write_int(out, source.get_int(record, col, e), c.bits / 8u);
          break;
        case ColumnType::Float:
          for (std::uint16_t e = 0; e < c.array_len; ++e)
            write_u32(out, source.get_slot(record, col, e));
          break;
        case ColumnType::String:
          for (std::uint16_t e = 0; e < c.array_len; ++e)
            write_u32(out, pool.resolve(source.get_string(record, col, e),
                                        state.string_offsets, cursor));
          break;
        case ColumnType::LocString:
          for (std::uint8_t loc = 0; loc < c.locale_count; ++loc)
            write_u32(out, pool.resolve(source.get_string(record, col, loc),
                                        state.string_offsets, cursor));
