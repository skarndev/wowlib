...
/** @file
    ...
    (tableHash/build/timestamp), an optional id-index block ahead of the records
    (engaged when maxId != 0) and an optional trailing copy table. Field types
...
namespace wowlib::db {
  ...
  struct Wdb2Header {
    std::uint32_t magic = Wdb2Magic; /**< "WDB2". */
    ...
    std::uint32_t timestampLastWritten = 0; /**< Unix timestamp, often 0. */
    std::uint32_t minId = 0; /**< Lowest record id; 0 when no index block. */
    std::uint32_t maxId = 0; /**< Highest record id; non-zero engages the index block. */
    std::uint32_t locale = 0; /**< Locale mask of the generating client. */
    std::uint32_t copyTableSize = 0; /**< Trailing copy-table bytes (id pairs). */
  };

  static_assert(sizeof(Wdb2Header) == 48 && std::is_trivially_copyable_v<Wdb2Header>);

  /** The per-id index block entry stride when maxId != 0:
      `int32 indices[maxId - minId + 1]` followed by
      `int16 string_lengths[maxId - minId + 1]` — 6 bytes per id in total. */
  inline constexpr std::size_t Wdb2IndexEntryBytes = 6;
