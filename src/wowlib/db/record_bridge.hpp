...
namespace wowlib::db {
  namespace detail {
    ...
    inline constexpr RecordOps RecordOpsFor{
      ...
      [](const void* v) {
        return static_cast<const std::vector<Record>*>(v)->size();
      },
      [](void* v, std::size_t i) {
        return reinterpret_cast<std::byte*>(&(*static_cast<std::vector<Record>*>(v))[i]);
      },
      [](const void* v, std::size_t i) {
        return reinterpret_cast<const std::byte*>(&(*static_cast<const std::vector<Record>*>(v))[i]);
      },
      ...
      [](void* v, const void* src) {
        static_cast<std::vector<Record>*>(v)->push_back(*static_cast<const Record*>(src));
      },
      [](void* v, std::size_t i, const void* src) {
        (*static_cast<std::vector<Record>*>(v))[i] = *static_cast<const Record*>(src);
      },
      [](void* v, std::size_t i) {
        auto& vec = *static_cast<std::vector<Record>*>(v);
