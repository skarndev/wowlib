...
namespace wowlib::db
{
                         ...
                         RecordSink& sink, TableState& state)
  {
    if (data.size() < sizeof(Wdb2Header))
      return make_error(ErrorCode::TableTruncated,
                        std::format("{}: {} bytes is too small for a WDB2 header", info.name,
                                    data.size()));
    Wdb2Header header;
    ...
    if (header.record_size != stride)
      return make_error(
        ...
        std::format("{}: file record_size {} disagrees with the generated schema stride {}",
                    info.name, header.record_size, stride));
    if (header.max_id != 0 && header.max_id < header.min_id)
      return make_error(ErrorCode::TableTruncated,
                        std::format("{}: WDB2 max_id {} below min_id {}", info.name,
    ...
    if (data.size() != expected)
      return make_error(
        ...
        std::format("{}: {} bytes on disk, but the header describes {} ({} records of {} bytes "
                    "+ {} index + {} string + {} copy-table bytes)",
                    info.name, data.size(), expected, header.record_count, header.record_size,
                    index_bytes, header.string_block_size, header.copy_table_size));

    sink.clear();
    state.reset();
    if (auto r = state.strings.read(data.subspan(
    ...
    state.string_offsets.reserve(std::size_t{header.record_count}
                                 * detail::string_slot_count(info.schema));
                                ...
                                const TableState& state)
  {
    ...
    Wdb2Header header{};
    if (loaded && state.wdb2_header.size() == sizeof header)
      std::memcpy(&header, state.wdb2_header.data(), sizeof header);

    if (loaded && !state.wdb2_index.empty()
        && source.size() != header.record_count)
      return make_error(
        ErrorCode::InvalidEntityState,
        std::format("{}: the WDB2 id-index block cannot be rebuilt yet; adding or removing "
                    "records of an indexed table is unsupported (had {}, have {})",
                    info.name, header.record_count, source.size()));
    ...
    if (!loaded)
      header.build = info.version.build;

    detail::StringPool pool{state.strings};
    FileBuffer out;
    out.reserve(sizeof header + state.wdb2_index.size() + source.size() * stride
                + pool.block.size() + state.wdb2_copy.size());
    ...
    if (auto w = pool.block.write(out); !w)
