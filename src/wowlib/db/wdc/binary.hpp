...
namespace wowlib::db::wdc {
  ...
  inline constexpr std::uint32_t wdc1_magic = formats::four_cc("WDC1", formats::FourCCEndian::forward);

  /** The WDC3 magic as memcpy'd off the file front (the bytes "WDC3"). */
  inline constexpr std::uint32_t wdc3_magic = formats::four_cc("WDC3", formats::FourCCEndian::forward);

  /** The WDC4 magic as memcpy'd off the file front (the bytes "WDC4"). */
  inline constexpr std::uint32_t wdc4_magic = formats::four_cc("WDC4", formats::FourCCEndian::forward);

  /** The WDC5 magic as memcpy'd off the file front (the bytes "WDC5"). */
  inline constexpr std::uint32_t wdc5_magic = formats::four_cc("WDC5", formats::FourCCEndian::forward);
  ...
  constexpr bool is_wdc_magic(std::uint32_t magic) {
    return magic == wdc1_magic || magic == wdc3_magic || magic == wdc4_magic || magic == wdc5_magic;
  }
