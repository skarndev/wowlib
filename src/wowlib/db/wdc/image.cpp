...
namespace wowlib::db::wdc
{
  ...
  Result<WdcImage> WdcImage::parse(std::span<const std::byte> data)
  {
    ...
    if (magic == wdc1_magic)
      return parse_wdc1(data);
    if (magic == wdc3_magic || magic == wdc4_magic || magic == wdc5_magic)
      return parse_wdc3(data, magic);
    return make_error(ErrorCode::TableMagicUnknown,
                      "WDC: the file magic is not a supported WDC flavor");
  }
  ...
  Result<WdcImage> WdcImage::parse_wdc1(std::span<const std::byte> data)
  {
    ...
    Wdc1Header h1;
    if (data.size() < sizeof h1)
      return fail("WDC1: file smaller than its header");
    ...
    const std::size_t field_struct_bytes =
      std::size_t{h1.field_count} * sizeof(WdcFieldStructure);
    if (data.size() < pos + field_struct_bytes)
      return fail("WDC1: field structure table overruns the file");
    img.field_structure.resize(h1.field_count);
    if (h1.field_count)
      take_raw(data, pos, img.field_structure.data(), field_struct_bytes);
    ...
    if (sparse)
    {
      ...
      if (h1.offset_map_offset < pos || h1.offset_map_offset > data.size())
        return fail("WDC1: sparse offset_map_offset out of range");
      ...
      const std::size_t dense_count =
        h1.max_id >= h1.min_id ? std::size_t{h1.max_id} - h1.min_id + 1 : 0;
      if (pos + dense_count * 6 > data.size())
        return fail("WDC1: dense offset map overruns the file");
      ...
      sec.header.offset_map_id_count =
        static_cast<std::uint32_t>(img.owned_offset_map_ids.size());
    }
    else
    {
      const std::size_t rec_bytes = std::size_t{h1.record_count} * h1.record_size;
      if (pos + rec_bytes + h1.string_table_size > data.size())
        return fail("WDC1: records+strings overrun the file");
    ...
    if (pos + h1.id_list_size + h1.copy_table_size + h1.field_storage_info_size
          + h1.pallet_data_size + h1.common_data_size + h1.relationship_data_size
        > data.size())
      return fail("WDC1: trailing blocks overrun the file");
    ...
    if (storage_count)
      take_raw(data, pos, img.field_storage.data(), h1.field_storage_info_size);
    ...
    for (WdcFieldStorage& fs : img.field_storage)
      if (fs.storage_type == WdcCompression::Bitpacked && (fs.val3 & 1u))
        fs.storage_type = WdcCompression::BitpackedSigned;
  ...
  Result<WdcImage> WdcImage::parse_wdc3(std::span<const std::byte> data, std::uint32_t magic)
  {
    ...
    if (data.size() < pos + prefix + header_rest)
      return fail("WDC: file smaller than its header");
    if (prefix)
      take_raw(data, pos, &img.wdc5, sizeof img.wdc5);
    ...
    if (data.size() < pos + sections_bytes + field_struct_bytes + h.field_storage_info_size
      ...
      return fail("WDC: header tables overrun the file");

    std::vector<Wdc3SectionHeader> section_headers(h.section_count);
    if (h.section_count)
      take_raw(data, pos, section_headers.data(), sections_bytes);
    ...
    img.field_storage.resize(storage_count);
    if (storage_count)
      take_raw(data, pos, img.field_storage.data(), h.field_storage_info_size);
    ...
    if (magic != wdc3_magic)
      ...
      {
        if (section_headers[s].tact_key_hash == 0)
          continue;
        std::uint32_t count = 0;
        if (pos + 4 > data.size())
          return fail("WDC: encrypted_status overruns the file");
        take_raw(data, pos, &count, 4);
        if (pos + std::size_t{count} * 4 > data.size())
          return fail("WDC: encrypted_status id list overruns the file");
        encrypted_ids[s].resize(count);
        if (count)
          take_raw(data, pos, encrypted_ids[s].data(), std::size_t{count} * 4);
      }
    ...
    for (std::uint32_t s = 0; s < h.section_count; ++s)
    {
      ...
      if (p > data.size())
        return fail("WDC: section file_offset past end");

      if (sparse)
      {
        if (sh.offset_records_end < sh.file_offset || sh.offset_records_end > data.size())
          return fail("WDC: sparse offset_records_end out of range");
      ...
      else
      {
        ...
        if (p + rec_bytes + sh.string_table_size > data.size())
          return fail("WDC: section records+strings overrun the file");
      ...
      const bool ids_after_relationship =
        magic != wdc3_magic && (h.flags & wdc_flag_secondary) == 0;
      if (!take(sh.id_list_size, sec.id_list))
        return fail("WDC: id_list overruns the file");
      if (!take(std::size_t{sh.copy_table_count} * 8, sec.copy_table))
        return fail("WDC: copy_table overruns the file");
      if (!take(std::size_t{sh.offset_map_id_count} * 6, sec.offset_map))
        return fail("WDC: offset_map overruns the file");
      if (!ids_after_relationship
          && !take(std::size_t{sh.offset_map_id_count} * 4, sec.offset_map_ids))
        return fail("WDC: offset_map id list overruns the file");
      if (!take(sh.relationship_data_size, sec.relationship))
        return fail("WDC: relationship block overruns the file");
      if (ids_after_relationship
          && !take(std::size_t{sh.offset_map_id_count} * 4, sec.offset_map_ids))
        return fail("WDC: offset_map id list overruns the file");
  ...
  std::vector<std::uint32_t> WdcImage::field_additional_offsets() const
  {
    ...
    for (std::size_t i = 0; i < field_storage.size(); ++i)
    {
      ...
      if (fs.storage_type == WdcCompression::Pallet
          || fs.storage_type == WdcCompression::PalletArray)
      {
        offsets[i] = pallet;
  ...
  std::uint64_t WdcImage::field_raw(std::size_t field, std::uint32_t element,
  ...
  std::uint32_t WdcImage::pallet_value(std::uint32_t base, std::uint32_t index,
                                       std::uint32_t element, std::uint32_t array_size) const
  {
  ...
  std::uint32_t WdcImage::common_value(std::uint32_t base, std::uint32_t size, std::uint32_t id,
