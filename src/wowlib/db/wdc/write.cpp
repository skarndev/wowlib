  ...
  namespace {
    ...
    public:
      ...
      Writer(std::uint32_t magic, const TableInfo& info, const RecordSource& source, const TableState& state)
        ...
        for (const Column& c : info.schema) {
          if (!c.noninline) cols_.emplace_back(colidx, c);
          else if (c.is_relation) relation_col_ = colidx;
          ++colidx;
        }
      }
      ...
      Result<FileBuffer> emit() {
        ...
        derive_common();
        encode_records();
        if (auto w = block_.write(string_region_); !w) return std::unexpected{w.error()};
        scan_id_range();
    ...
    private:
      ...
      void plan_fields() {
        ...
        for (std::size_t f = 0; f < cols_.size(); ++f) {
          ...
          const auto kind = f < state_.wdc_kinds.size()
                              ...
                              : WdcCompression::None;
          const bool orig_pallet = kind == WdcCompression::Pallet || kind == WdcCompression::PalletArray;
          ...
          if (palletable && col.array_len == 1 && orig_common) p.storage = WdcCompression::CommonData;
            ...
            p.storage = col.is_signed ? WdcCompression::BitpackedSigned : WdcCompression::Bitpacked;
          else p.storage = WdcCompression::None;

          if (p.is_pallet()) p.elem_bits = 1; // real width set below, from the distinct count
          else if (p.is_common()) p.elem_bits = 0;
          else if (col.type == ColumnType::Int)
            p.elem_bits = std::min<std::uint16_t>(
              col.is_signed ? signed_width(lo[f], hi[f]) : unsigned_width(static_cast<std::uint64_t>(hi[f])), col.bits);
          else p.elem_bits = 32;
        }
        ...
        for (std::size_t r = 0; r < source_.size(); ++r)
          for (std::size_t f = 0; f < cols_.size(); ++f)
            if (plan_[f].is_pallet()) {
              ...
              if (plan_[f].pallet_index.try_emplace(key, static_cast<std::uint32_t>(plan_[f].pallet_index.size())).
                           second)
      ...
      void layout_record() {
        ...
        for (FieldPlan& p : plan_) {
          if (p.storage == WdcCompression::None) record_bits = (record_bits + 7) / 8 * 8;
      ...
      void derive_copies() {
        ...
        if (record_size_ >= 8) {
          ...
          for (std::uint32_t r = 0; r < record_count; ++r) {
            ...
            if (const auto it = first.find(key); it != first.end()) copies_.emplace_back(id, it->second);
            else {
          ...
          for (std::uint32_t r = 0; r < record_count; ++r) reals_[r] = r;
        }
      }
      ...
      void derive_common() {
        ...
        for (const std::uint32_t ri : reals_) {
          ...
          for (std::size_t f = 0; f < cols_.size(); ++f)
            if (plan_[f].is_common()) raw[f].emplace_back(id, source_.get_slot(ri, cols_[f].first, 0));
        }
        for (std::size_t f = 0; f < cols_.size(); ++f) {
          if (!plan_[f].is_common()) continue;
          ...
          for (const auto& [id, v] : raw[f])
            if (v != def) common_[f].emplace_back(id, v);
          std::ranges::sort(common_[f]);
        }
        for (const auto& entries : common_) common_total_ += static_cast<std::uint32_t>(entries.size() * 8);
        for (const FieldPlan& p : plan_)
          if (p.is_pallet()) pallet_total_ += static_cast<std::uint32_t>(p.pallet.size() * 4);
      }
      ...
      void encode_record(std::size_t r, BitWriter& writer, std::size_t record_abs) {
        for (std::size_t f = 0; f < cols_.size(); ++f) {
          ...
          if (p.is_common()) continue; // the value lives in common_data only
          if (p.is_pallet()) {
            const std::string key = pallet_key(r, f);
            const auto it = p.pallet_index.find(key);
            writer.write(p.offset_bits, p.elem_bits, it != p.pallet_index.end() ? it->second : 0);
            continue;
          }
          for (std::uint16_t e = 0; e < col.array_len; ++e) {
            ...
            if (col.type == ColumnType::String) {
              ...
              if (const auto it = lookup_.find(value); it != lookup_.end()) block_off = it->second;
              else {
              ...
              // WDC1 string references are plain offsets into the block.
                ...
                writer.write(bit_off, 32, static_cast<std::uint32_t>(strings_at_ + block_off - field_abs));
              }
            }
            else if (col.type == ColumnType::Float) writer.write(bit_off, 32, source_.get_slot(r, colidx, e));
            else // Int: bitpacked (signed values keep their low bits)
            {
              const std::uint64_t mask = p.elem_bits >= 64 ? ~std::uint64_t{0} : (std::uint64_t{1} << p.elem_bits) - 1;
              writer.write(bit_off, p.elem_bits, static_cast<std::uint64_t>(source_.get_int(r, colidx, e)) & mask);
            }
          }
        }
      }
      ...
      std::size_t frame_bytes_before_records() const {
        ...
        return prefix + sizeof(Wdc3Header) + sizeof(Wdc3SectionHeader) + field_count * sizeof(WdcFieldStructure) +
          field_count * sizeof(WdcFieldStorage) + pallet_total_ + common_total_;
      }

      /** Whether the schema's id column is non-inline (flag 0x04). */
      bool noninline_id() const {
        return db::detail::id_is_noninline(info_.schema);
      }
      ...
      std::size_t relationship_bytes() const {
        return relation_col_ == std::numeric_limits<std::size_t>::max() ? 0 : 12 + reals_.size() * 8;
      }
      ...
      void append_field_storage(FileBuffer& out) const {
        ...
        for (const FieldPlan& p : plan_) {
          ...
          fs.field_size_bits = static_cast<std::uint16_t>(p.is_pallet()
                                                            ? p.elem_bits
                                                            : (p.is_common()
                                                                 ? 0
                                                                 : std::size_t{p.elem_bits} * p.elements));
          fs.storage_type = p.storage;
          if (p.storage == WdcCompression::Bitpacked || p.storage == WdcCompression::BitpackedSigned || p.is_pallet()) {
      ...
      void append_pallet_common(FileBuffer& out) const {
        for (const FieldPlan& p : plan_)
          if (p.is_pallet())
            for (const std::uint32_t v : p.pallet) append(out, &v, 4);
      ...
      void append_ids_copies(FileBuffer& out) const {
        if (noninline_id())
          for (const std::uint32_t id : ids_) append(out, &id, 4);
      ...
      Result<FileBuffer> emit_wdc1() const {
        ...
        header.id_index = noninline_id() ? std::uint16_t{0} : db::detail::id_field_index(info_.schema);
        ...
        header.field_storage_info_size = header.field_count * static_cast<std::uint32_t>(sizeof(WdcFieldStorage));
        ...
        FileBuffer out;
        out.reserve(
          strings_at_ + string_region_.size() + header.id_list_size + header.copy_table_size + header.
          field_storage_info_size + pallet_total_ + common_total_ + header.relationship_data_size);
      ...
      Result<FileBuffer> emit_wdc3() const {
        ...
        header.id_index = has_id_list ? std::uint16_t{0} : db::detail::id_field_index(info_.schema);
        header.total_field_count = field_count;
        header.bitpacked_data_offset = bitpacked_at_;
        header.lookup_column_count = relationship_bytes() ? 1 : 0;
        header.field_storage_info_size = field_count * static_cast<std::uint32_t>(sizeof(WdcFieldStorage));
        header.pallet_data_size = pallet_total_;
        header.common_data_size = common_total_;
        ...
        FileBuffer out;
        out.reserve(
          strings_at_ + string_region_.size() + std::size_t{real_count} * 4
        ...
        else append(out, &header, sizeof header);
      ...
      void append_relationship(FileBuffer& out) const {
        if (relation_col_ == std::numeric_limits<std::size_t>::max()) return;
        ...
        for (std::uint32_t i = 0; i < reals_.size(); ++i) {
          const auto foreign = static_cast<std::uint32_t>(source_.get_int(reals_[i], relation_col_, 0));
