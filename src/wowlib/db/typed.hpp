...
#embed <wowlib_schema.wdbs>
    ...
    /** The blob's column list for (@a table, @a version)'s era, as
        ...
        @ref typed_mismatch. */
    consteval blob::RangeEntry typed_range_of(std::string_view table,
                                              ClientVersion version)
    {
      const blob::View view{std::span<const unsigned char>{typed_schema_blob}};
      const auto at = view.find_table(table);
      if (!at)
        throw typed_mismatch{"the schema blob knows no table of this name"};
      std::size_t era = SIZE_MAX;
      for (std::size_t i = 0; i < view.era_count(); ++i)
        if (view.era(i).major == version.major)
          era = i;
      if (era == SIZE_MAX)
        throw typed_mismatch{
          "no targeted era shares the struct's major version"};
      ...
      for (std::size_t r = 0; r < entry.range_count; ++r)
      {
        ...
        if (range.era_mask & (std::uint16_t{1} << era))
          return range;
      }
      throw typed_mismatch{
        "the table has no schema range covering the struct's era"};
    }

    /** The era-schema column index whose name is @a name, or a thrown
        @ref typed_mismatch. */
    consteval std::size_t typed_column_index(std::string_view table,
                                             ClientVersion version,
                                             std::string_view name)
    {
      ...
      for (std::size_t c = 0; c < range.column_count; ++c)
        if (view.copy_string_at(
                view.column(range.first_column + c).name_off) == name)
          return c;
      throw typed_mismatch{
        "the era's schema has no column with this member's name"};
    }
    ...
    {
      const blob::View view{std::span<const unsigned char>{typed_schema_blob}};
      const blob::RangeEntry range =
        typed_range_of(Record::table_name, Record::version);
      for (const Column& member : schema_of<Record>())
      {
        const std::size_t at = typed_column_index(
          Record::table_name, Record::version, member.name_view());
        const blob::ColumnEntry col = view.column(range.first_column + at);
        if (col.type != member.type)
          throw typed_mismatch{"the member's value class (Int/Float/String/"
                               "LocString) differs from the DBD column's"};
        if (member.type == ColumnType::Int &&
            (col.bits != member.bits || col.is_signed != member.is_signed))
          throw typed_mismatch{"the member's integer width or signedness "
                               "differs from the DBD column's"};
        if (col.array_len != member.array_len)
          throw typed_mismatch{
            "the member's array length differs from the DBD column's"};
        if (member.type == ColumnType::LocString &&
            col.locale_count != member.locale_count)
          throw typed_mismatch{"the member's LocString locale count differs "
                               "from the DBD column's (wrong era?)"};
      }
      return true;
    }
    ...
    consteval auto typed_projection()
    {
      ...
      for (const Column& member : schema_of<Record>())
        map.push_back(typed_column_index(Record::table_name, Record::version,
                                         member.name_view()));
  ...
  public:
    ...
    {
      return DynTable::open(Record::table_name, Record::version)
        .transform([](DynTable&& table) {
          TypedTable out{};
    ...
    std::vector<Record> load() const
    {
      ...
      for (std::size_t r = 0; r < out.size(); ++r)
        for (std::size_t m = 0; m < schema.size(); ++m)
          copy_cell_in(sink, r, m, projection[m], schema[m]);
      return out;
    }
    ...
    void write_back(const std::vector<Record>& records)
      requires(schema_of<Record>().size() ==
               detail::typed_range_of(Record::table_name,
                                      Record::version).column_count)
    {
      table_.clear_rows();
      ...
      for (std::size_t r = 0; r < records.size(); ++r)
      {
        table_.append_row();
        for (std::size_t m = 0; m < schema.size(); ++m)
          copy_cell_out(source, r, m, projection[m], schema[m]);
      }
    }
    ...
    auto column() const
    {
      ...
      constexpr std::size_t index = []() consteval {
        for (std::size_t i = 0; i < members.size(); ++i)
          if (std::meta::identifier_of(members[i]) == Name.view())
            return i;
  ...
  private:
    ...
    void copy_cell_in(ErasedRecordSink& sink, std::size_t r, std::size_t m,
                      std::size_t col, const Column& shape) const
    {
      switch (shape.type)
      {
        case ColumnType::Int:
          for (std::size_t e = 0; e < shape.array_len; ++e)
            sink.set_int(r, m, e, table_.get_int(r, col, e).value());
          break;
        case ColumnType::Float:
          for (std::size_t e = 0; e < shape.array_len; ++e)
            sink.set_float(r, m, e, table_.get_float(r, col, e).value());
          break;
        case ColumnType::String:
          for (std::size_t e = 0; e < shape.array_len; ++e)
            sink.set_string(r, m, e, table_.get_string(r, col, e).value());
          break;
        case ColumnType::LocString:
          for (std::size_t e = 0; e < shape.locale_count; ++e)
            sink.set_string(r, m, e, table_.get_string(r, col, e).value());
    ...
    void copy_cell_out(const ErasedRecordSource& source, std::size_t r,
                       std::size_t m, std::size_t col, const Column& shape)
    {
      switch (shape.type)
      {
        case ColumnType::Int:
          for (std::size_t e = 0; e < shape.array_len; ++e)
            table_.set_int(r, col, source.get_int(r, m, e), e).value();
          break;
        case ColumnType::Float:
          for (std::size_t e = 0; e < shape.array_len; ++e)
            table_.set_float(
              r, col, std::bit_cast<float>(source.get_slot(r, m, e)), e)
              .value();
          break;
        case ColumnType::String:
          for (std::size_t e = 0; e < shape.array_len; ++e)
            table_.set_string(r, col, source.get_string(r, m, e), e).value();
          break;
        case ColumnType::LocString:
          for (std::size_t e = 0; e < shape.locale_count; ++e)
            table_.set_string(r, col, source.get_string(r, m, e), e).value();
          table_
            .set_locstring_flags(
              r, col, static_cast<std::uint32_t>(source.get_int(r, m, 0)))
            .value();
          break;
      }
    }
