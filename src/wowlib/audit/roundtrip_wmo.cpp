...
namespace {
  ...
  RoundtripReport roundtrip_wmo(fs::FileSystem& fs, const std::string& path) {
    ...
    if (auto outcome = audit::detail::classify_read(report, "root read", root_data)) return *outcome;

    wmo::WMORoot < V > root;
    if (auto r = root.read(*root_data); !r) return audit::detail::fail_with(report, "root parse", r.error().message);
    ...
    if (auto divergence = audit::detail::first_divergence_chunked(*root_data, *rewritten_root)) return
      audit::detail::fail_with(report, "root compare", *divergence);
    ...
    for (std::size_t i = 0; i < n_groups; ++i) {
      ...
      if (auto outcome = audit::detail::classify_read(report, std::format("group {} read", i), group_data)) return *
        outcome;
      ...
      if (!rewritten)
        return audit::detail::fail_with(report, std::format("group {} write", i), rewritten.error().message);
...
namespace wowlib::audit::detail {
  RoundtripReport FormatDrivers::wmo(fs::FileSystem& fs, const std::string& path, ClientVersion version) {
    ...
    const bool matched = with_version(version, [&]<ClientVersion V>() {
      if constexpr (version_supported(formats::wmo::wmo_versions, V)) report = guarded([&] {
        return roundtrip_wmo<V>(fs, path);
      });
    });
