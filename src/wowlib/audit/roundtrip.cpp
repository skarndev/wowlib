...
namespace
{
  ...
  bool is_wmo_group(std::string_view canonical)
  {
    ...
    return stem_end >= 4 && canonical[stem_end - 4] == '_'
           ...
           && std::isdigit(static_cast<unsigned char>(canonical[stem_end - 2]))
           && std::isdigit(static_cast<unsigned char>(canonical[stem_end - 1]));
  }
  ...
  bool is_aux_wdt(std::string_view stem)
  {
    for (const std::string_view suffix :
         {"_occ", "_lgt", "_fogs", "_mpv", "_tex", "_wmo", "_psd", "_pd4"})
...
{
  RoundtripReport Auditor::roundtrip(fs::FileSystem& fs, std::string_view path,
                                     ClientVersion version)
  {
    ...
    {
      if (is_wmo_group(view))
        return audit::detail::skipped("wmo-group");
      ...
      if (const auto lod = stem.rfind("_lod");
          lod != std::string_view::npos && lod + 4 < stem.size()
    ...
    if (view.ends_with(".m2"))
      return audit::detail::FormatDrivers::m2(fs, canonical, version);

    // Pre-M2 model spellings; sighted in early clients but not parsed here.
    if (view.ends_with(".mdx") || view.ends_with(".mdl"))
      return audit::detail::skipped("mdx");

    if (view.ends_with(".wdt"))
    {
      if (is_aux_wdt(view.substr(0, view.size() - 4)))
        return audit::detail::skipped("aux-wdt");
      return audit::detail::FormatDrivers::wdt(fs, canonical, version);
    }

    if (view.ends_with(".wdl"))
      return audit::detail::FormatDrivers::wdl(fs, canonical, version);

    if (view.ends_with(".blp"))
      return audit::detail::FormatDrivers::blp(fs, canonical);

    if (view.ends_with(".adt"))
    {
      if (is_adt_split_file(view.substr(0, view.size() - 4)))
        return audit::detail::skipped("adt-split-file");
