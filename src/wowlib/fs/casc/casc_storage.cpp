...
namespace wowlib::fs
{
  namespace
  {
    ...
    std::vector<BuildConfigCandidate> scan_build_configs(const fsys::path& config_dir)
    {
      ...
      std::error_code ec;
      for (fsys::recursive_directory_iterator it{config_dir, ec}, end; !ec && it != end;
           it.increment(ec))
      {
        ...
        std::string line;
        if (!std::getline(file, line) || !line.starts_with("# Build Configuration"))
          continue;
        ...
        while (std::getline(file, line))
          if (const auto pos = line.find("build-name"); pos != std::string::npos)
          {
            if (const auto wow = line.find("WOW-"); wow != std::string::npos)
              std::from_chars(line.data() + wow + 4, line.data() + line.size(),
                              candidate.build);
            break;
          }
    ...
    {
      const auto shim = fsys::temp_directory_path() / "wowlib-casc-shim" /
                        std::format("{:016x}-{}", fsys::hash_value(client_data),
                                    build_key);
      ...
      if (ec)
        return make_error(ErrorCode::IoError,
                          std::format("cannot create CASC shim '{}': {}", shim.string(),
                                      ec.message()));
      ...
      if (ec)
        return make_error(ErrorCode::IoError,
                          std::format("cannot link '{}' into CASC shim: {}",
                                      client_data.string(), ec.message()));
      ...
      if (!info.flush())
        return make_error(ErrorCode::IoError,
                          std::format("cannot write '{}'", (shim / ".build.info").string()));
    ...
    Result<FileBuffer> read_open_file(HANDLE file, const std::string& what)
    {
      ...
      if (!CascGetFileSize64(file, &size))
      {
        ...
        return make_error(ErrorCode::BackendError,
                          std::format("CascGetFileSize64 failed for {}", what),
      ...
      DWORD read = 0;
      if (!buffer.empty() &&
          !CascReadFile(file, buffer.data(), static_cast<DWORD>(buffer.size()), &read))
      {
        const auto native = GetCascError();
        CascCloseFile(file);
        ...
        return make_error(encrypted ? ErrorCode::EncryptedContent : ErrorCode::BackendError,
                          encrypted
                            ...
                            : std::format("CascReadFile failed for {}", what),
                          static_cast<std::uint32_t>(native));
      }
