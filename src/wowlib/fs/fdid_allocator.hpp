  ...
  public:
    /** @param start the first id to hand out.
        @param max   the last usable id (inclusive); defaults to just below the
                     u32 ceiling. */
    explicit FdidAllocator(FileDataID start = FileDataID{1'000'000'000}, FileDataID max = FileDataID{0xFFFFFFFE})
      : _next(start.value), _max(max.value) {}
    ...
    FileDataID peek() const {
      return FileDataID{static_cast<std::uint32_t>(_next)};
    }

  private:
    std::uint64_t _next;
    // u64 internally so _next can pass _max without wrapping
    std::uint64_t _max;
  };
}
