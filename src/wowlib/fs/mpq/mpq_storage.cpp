...
namespace {
  ...
  wowlib::Result<wowlib::FileBuffer> readLooseFile(const fsys::path& path) {
    std::ifstream in{path, std::ios::binary | std::ios::ate};
    if (!in)
      return wowlib::makeError(wowlib::ErrorCode::BackendError,
                                std::format("failed to open loose file '{}'", path.string()));
    const std::streamoff size = in.tellg();
    if (size < 0)
      return wowlib::makeError(wowlib::ErrorCode::BackendError,
                                std::format("failed to size loose file '{}'", path.string()));
    wowlib::FileBuffer buffer(static_cast<std::size_t>(size));
    in.seekg(0);
    if (!buffer.empty() && !in.read(reinterpret_cast<char*>(buffer.data()), size))
      return wowlib::makeError(wowlib::ErrorCode::BackendError,
                                std::format("failed to read loose file '{}'", path.string()));
    return buffer;
  }
  ...
  bool isSyntheticName(std::string_view canonical) {
    for (const std::string_view metadata : {"(listfile)", "(attributes)", "(signature)", "(patch_metadata)"})
      if (canonical == metadata) return true;

    if (canonical.size() < 13 || !canonical.starts_with("file") || canonical[12] != '.') return false;
    for (std::size_t i = 4; i < 12; ++i)
      if (canonical[i] < '0' || canonical[i] > '9') return false;
    return true;
  }
}
...
namespace wowlib::fs {
  Result<MpqStorage> MpqStorage::open(Options options) {
    ...
    if (auto opened = storage._openChain(); !opened) return std::unexpected(opened.error());
    return storage;
  }

  Result<void> MpqStorage::_openChain() {
    const detail::MpqChainSpec* spec = detail::findChainSpec(_options.version);
    if (!spec)
      return makeError(ErrorCode::StorageOpenFailed, std::format("no MPQ chain table for client {}.{}.{} (build {})",
    ...
    // retrofit a Data/{code}/ tier — expandChain mounts it when it exists.
    const std::string code{localeCode(_options.locale)};
    std::error_code ec;
    if (_options.version.major >= 2 && !std::filesystem::is_directory(_options.dataDir / code, ec))
      return makeError(ErrorCode::StorageOpenFailed,
                        std::format("locale directory '{}' not found under '{}'", code, _options.dataDir.string()));

    auto chain = detail::expandChain(*spec, _options.dataDir, _options.locale);
    if (!chain) return std::unexpected(chain.error());
    if (chain->empty())
      return makeError(ErrorCode::StorageOpenFailed, std::format("no archives of the {}.{}.{} chain exist under '{}'",
                                                                  _options.version.major, _options.version.minor,
                                                                  _options.version.patch, _options.dataDir.string()));
    ...
    for (const detail::ChainMember& member : *chain) {
      if (member.incremental) {
        ...
        const std::string patchPath = member.path.string();
        for (OpenedArchive& archive : _archives) {
          if (archive.isDirectory || archive.path.parent_path() != member.path.parent_path()) continue;
          if (!SFileOpenPatchArchive(archive.handle, patchPath.c_str(), nullptr, 0)) {
            const auto native = SErrGetLastError();
            _close();
            return makeError(ErrorCode::ArchiveOpenFailed,
                              std::format("SFileOpenPatchArchive failed attaching '{}' to '{}'", member.path.string(),
                                          archive.path.string()), static_cast<std::uint32_t>(native));
          }
      ...
      if (member.isDirectory) {
        ...
        // lowercased/backslashed by normalizePath).
        OpenedArchive slot{member.path, true};
        std::error_code walkEc;
        for (const auto& entry : std::filesystem::recursive_directory_iterator{member.path, walkEc}) {
          if (!entry.is_regular_file(walkEc)) continue;
          const auto relative = std::filesystem::relative(entry.path(), member.path, walkEc);
          if (walkEc) continue;
          slot.loose.emplace(normalizePath(relative.generic_string()), entry.path());
        }
      ...
      constexpr DWORD openFlags = MPQ_OPEN_READ_ONLY | MPQ_OPEN_NO_LISTFILE | MPQ_OPEN_NO_ATTRIBUTES |
        MPQ_OPEN_NO_HEADER_SEARCH;

      HANDLE handle = nullptr;
      const std::string archivePath = member.path.string();
      if (!SFileOpenArchive(archivePath.c_str(), 0, openFlags, &handle)) {
        const auto native = SErrGetLastError();
        _close();
        return makeError(ErrorCode::ArchiveOpenFailed,
                          std::format("SFileOpenArchive failed for '{}'", member.path.string()),
                          static_cast<std::uint32_t>(native));
      }
      _archives.push_back(OpenedArchive{.path = member.path, .handle = handle, .mtx = std::make_unique<std::mutex>()});
    }
    return {};
  }

  void MpqStorage::_close() noexcept {
    for (auto& archive : _archives)
      if (archive.handle) SFileCloseArchive(archive.handle);
    _archives.clear();
  }

  Result<FileBuffer> MpqStorage::readFile(const FileKey& key) {
    if (!_isOpen()) return makeError(ErrorCode::StorageNotOpen, "MPQ storage is not open");
    if (!key.path)
      return makeError(ErrorCode::FdidNotResolvable,
                        "MPQ storage is path-addressed; resolve the FileDataID to a " "path through a listfile first");
    ...
    for (const OpenedArchive& archive : _archives | std::views::reverse) {
      if (archive.isDirectory) {
        ...
        if (it == archive.loose.end()) continue;
        return readLooseFile(it->second);
      }
      ...
      if (!archive.patched && !SFileHasFile(archive.handle, name.c_str())) continue;

      HANDLE file = nullptr;
      if (!SFileOpenFileEx(archive.handle, name.c_str(), SFILE_OPEN_FROM_MPQ, &file)) {
        const auto native = SErrGetLastError();
        if (archive.patched && native == ERROR_FILE_NOT_FOUND) continue;
        return makeError(ErrorCode::BackendError,
                          std::format("SFileOpenFileEx failed for '{}' in '{}'", name, archive.path.string()),
                          static_cast<std::uint32_t>(native));
      }

      DWORD sizeHigh = 0;
      const DWORD sizeLow = SFileGetFileSize(file, &sizeHigh);
      if (sizeLow == SFILE_INVALID_SIZE) {
        const auto native = SErrGetLastError();
        SFileCloseFile(file);
        return makeError(ErrorCode::BackendError, std::format("SFileGetFileSize failed for '{}'", name),
                          static_cast<std::uint32_t>(native));
      }

      FileBuffer buffer((static_cast<std::uint64_t>(sizeHigh) << 32) | sizeLow);
      DWORD read = 0;
      if (!buffer.empty() && !SFileReadFile(file, buffer.data(), static_cast<DWORD>(buffer.size()), &read, nullptr)) {
        const auto native = SErrGetLastError();
        SFileCloseFile(file);
        return makeError(ErrorCode::BackendError, std::format("SFileReadFile failed for '{}'", name),
                          static_cast<std::uint32_t>(native));
      }
    ...
    return makeError(ErrorCode::FileNotFound, std::format("'{}' was not found in the MPQ chain", name));
  }

  bool MpqStorage::exists(const FileKey& key) {
    if (!_isOpen() || !key.path) return false;

    for (const OpenedArchive& archive : _archives | std::views::reverse) {
      if (archive.isDirectory) {
        if (archive.loose.contains(*key.path)) return true;
        continue;
      }
      ...
      if (archive.patched) {
        ...
        // readFile for why the has-file check is blind to them).
        HANDLE file = nullptr;
      ...
      if (SFileHasFile(archive.handle, key.path->c_str())) return true;
    }
    return false;
  }

  Result<std::vector<std::string>> MpqStorage::enumeratePaths() {
    if (!_isOpen()) return makeError(ErrorCode::StorageNotOpen, "MPQ storage is not open");
    ...
    for (const OpenedArchive& archive : _archives) {
      if (archive.isDirectory) {
        ...
        for (const auto& name : archive.loose | std::views::keys) paths.insert(name);
        continue;
      }

      std::scoped_lock lock{*archive.mtx};

      // Archives open with MPQ_OPEN_NO_LISTFILE (see _openChain), so the name
      ...
      if (!find) continue; // nothing enumerable in this archive — skip it silently
      do {
        std::string canonical = normalizePath(found.cFileName);
        if (!isSyntheticName(canonical)) paths.insert(std::move(canonical));
      }
      while (SFileFindNextFile(find, &found));
      SFileFindClose(find);
    }

    return std::vector<std::string>{std::make_move_iterator(paths.begin()), std::make_move_iterator(paths.end())};
  }
}
