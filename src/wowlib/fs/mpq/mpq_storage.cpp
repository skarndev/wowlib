...
namespace wowlib::fs {
  Result<MpqStorage> MpqStorage::open(Options options) {
    ...
    if (auto opened = storage.open_chain(); !opened) return std::unexpected(opened.error());
    return storage;
  }
  ...
  Result<void> MpqStorage::open_chain() {
    ...
    if (!spec)
      return make_error(ErrorCode::StorageOpenFailed, std::format("no MPQ chain table for client {}.{}.{} (build {})",
    ...
    std::error_code ec;
    if (_options.version.major >= 2 && !std::filesystem::is_directory(_options.data_dir / code, ec))
      return make_error(ErrorCode::StorageOpenFailed,
                        std::format("locale directory '{}' not found under '{}'", code, _options.data_dir.string()));

    auto chain = detail::expand_chain(*spec, _options.data_dir, _options.locale);
    if (!chain) return std::unexpected(chain.error());
    if (chain->empty())
      return make_error(ErrorCode::StorageOpenFailed, std::format("no archives of the {}.{}.{} chain exist under '{}'",
                                                                  _options.version.major, _options.version.minor,
    ...
    for (const detail::ChainMember& member : *chain) {
      if (member.incremental) {
        ...
        for (OpenedArchive& archive : _archives) {
          if (archive.is_directory || archive.path.parent_path() != member.path.parent_path()) continue;
      ...
      if (member.is_directory) {
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator{member.path, walk_ec}) {
          if (!entry.is_regular_file(walk_ec)) continue;
          const auto relative = std::filesystem::relative(entry.path(), member.path, walk_ec);
          if (walk_ec) continue;
      ...
      constexpr DWORD open_flags = MPQ_OPEN_READ_ONLY | MPQ_OPEN_NO_LISTFILE | MPQ_OPEN_NO_ATTRIBUTES |
  ...
  Result<FileBuffer> MpqStorage::read_file(const FileKey& key) {
    if (!is_open()) return make_error(ErrorCode::StorageNotOpen, "MPQ storage is not open");
    if (!key.path)
    ...
    for (const OpenedArchive& archive : _archives | std::views::reverse) {
      ...
      if (size_low == SFILE_INVALID_SIZE) {
        const auto native = SErrGetLastError();
        SFileCloseFile(file);
        return make_error(ErrorCode::BackendError, std::format("SFileGetFileSize failed for '{}'", name),
                          static_cast<std::uint32_t>(native));
      }
      ...
      if (!buffer.empty() && !SFileReadFile(file, buffer.data(), static_cast<DWORD>(buffer.size()), &read, nullptr)) {
        const auto native = SErrGetLastError();
        SFileCloseFile(file);
        return make_error(ErrorCode::BackendError, std::format("SFileReadFile failed for '{}'", name),
                          static_cast<std::uint32_t>(native));
      }
    ...
    return make_error(ErrorCode::FileNotFound, std::format("'{}' was not found in the MPQ chain", name));
  }

  bool MpqStorage::exists(const FileKey& key) {
    if (!is_open() || !key.path) return false;
  ...
  Result<std::vector<std::string>> MpqStorage::enumerate_paths() {
    ...
    for (const OpenedArchive& archive : _archives) {
      ...
      do {
        ...
        if (!is_synthetic_name(canonical)) paths.insert(std::move(canonical));
      }
