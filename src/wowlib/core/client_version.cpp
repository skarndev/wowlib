...
namespace wowlib
{
  ...
  {
    constexpr std::array locales{Locale::enUS, Locale::koKR, Locale::frFR, Locale::deDE,
                                 Locale::zhCN, Locale::esES, Locale::zhTW, Locale::enGB,
                                 Locale::esMX, Locale::ruRU, Locale::ptBR, Locale::itIT};
    for (std::size_t i = 0; i < locale_table.size(); ++i)
      if (locale_table[i].first == code)
        return locales[i];
    return std::nullopt;
  }
