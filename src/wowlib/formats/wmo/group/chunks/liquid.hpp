...
namespace wowlib::formats::wmo::group::chunks
{
  ...
  ]] SMOLVert
  {
    ...
    [[=welder::doc("Water: flow-1 percentage. Magma/slime: low byte of int16 t.")]]
  ...
  ]] MLIQData
  {
    ...
    Result<void> read(std::span<const std::byte> payload)
    {
      ...
      const std::size_t n_verts =
        static_cast<std::size_t>(verts_dim.x) * static_cast<std::size_t>(verts_dim.y);
      const std::size_t n_tiles =
        static_cast<std::size_t>(tiles_dim.x) * static_cast<std::size_t>(tiles_dim.y);
    ...
    [[nodiscard]]
    [[=welder::getter, =welder::doc("Whether this group carries no liquid grid.")]]
