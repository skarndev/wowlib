...
namespace wowlib::formats::wmo::group::chunks {
  ...
  namespace detail {
    ...
    template <ClientVersion V> requires(V < wmo_split_groups)
    struct [[
      =welder::weld,
      =welder::doc(
  ...
  template <ClientVersion V>
  using SMOGroupHeader = detail::SMOGroupHeader<canonical_version(V, wmo_group_header_pivots, wmo_versions)>;
