...
/** @file
    The reflection-driven offset serializer: the engine behind
    OffsetFile::read()/write(). detail::read_offset_entity /
    detail::write_offset_entity walk an offset entity's members (`template
    for` over the reflected member list) and map them onto the M2-family wire
    layout: inline scalars plus `M2Array{count, offset}` references whose data
    blocks live elsewhere in the buffer.

    Member kinds are type-driven, mirroring the chunk serializer:
    ...
    depth-first in wire order, 16-byte aligned); there is no byte-perfect
    round-trip for offset formats — the guarantee is that a written entity
    re-reads equal. */
