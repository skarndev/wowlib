...
namespace wowlib::formats {
  namespace detail {
    ...
    struct ChunkSpec {
      std::uint32_t magic; /**< Comparison value, see fourCc(). */
    ...
    struct RepeatingSpec {};

    /** Stored form of `SequenceData` (offset entities): the member's nested
    ...
    struct SequenceDataSpec {};

    /** Stored form of `gatedBy` (offset entities): the member occupies binary
        bytes only when the entity's `globalFlags` has any @a mask bit set. */
    struct GatedBySpec {
      std::uint32_t mask;
    };

    /** Stored form of `offsetAfter` (offset entities): the trait-base member's
    ...
    /** Stored form of `countMatches`: when the member is engaged, its element
        count times @a scale must equal the named sibling member's count. */
    struct CountMatchesSpec {
      const char* name; /**< The sibling member (interned, see OffsetAfterSpec). */
      std::uint32_t scale; /**< This member's count is 1/scale of the sibling's. */

      constexpr std::string_view view() const { return name; }
    };

    /** Stored form of `countMultipleOf`: the member's element count must be
        a multiple of @a divisor. */
    struct CountMultipleOfSpec {
      std::uint32_t divisor;
    };

    /** Stored form of `countExactly`: the member's element count is fixed by
        the format (an ADT map chunk's 145 height samples). */
    struct CountExactlySpec {
      std::uint32_t count;
    };
    ...
    /** Stored form of `indexesOptional`: like `indexes`, but the client's
        "no reference" sentinel (a negative value, or the all-ones value of an
        unsigned element) is a legal element. */
    struct IndexesOptionalSpec {
      const char* name; /**< The sibling member the elements index. */

      constexpr std::string_view view() const { return name; }
    };

    /** Stored form of `indexesInRoot`: every element of the (integral)
    ...
    struct IndexesInRootSpec {
      const char* name; /**< The root-entity member the elements index. */

      constexpr std::string_view view() const { return name; }
    };

    /** Stored form of `expectedValue`: the (integral) data member must hold
  ...
  consteval detail::SinceSpec since(ClientVersion v) { return {v}; }

  /** Restrict a member to entity versions < @a v (exclusive).
      @param v the first client version the chunk no longer exists in. */
  consteval detail::UntilSpec until(ClientVersion v) { return {v}; }
  ...
  inline constexpr detail::OptionalSpec Optional{};
  ...
  inline constexpr detail::HeaderSpec Header{};
  ...
  inline constexpr detail::ContainerSpec Container{};
  ...
  consteval detail::RepeatsSpec repeats(std::uint32_t max) { return {max}; }
  ...
  inline constexpr detail::SequenceDataSpec SequenceData{};

  /** Make an offset-entity member's binary presence conditional on the entity's
      `globalFlags`: it occupies bytes only when `globalFlags & mask` is
  ...
  /** Declare a companion-count contract: when this member is engaged
      ...
      sibling member's count. Examples: normals `countMatches("vertices")`
      (one normal per vertex); polys `countMatches("indices", 3)` (one
  ...
  /** Declare a cross-entity referential contract: every element of this
      ...
      ASSEMBLY's root entity (a WMO group's lightRefs into the root's
  ...
  consteval detail::IndexesInRootSpec
  indexesInRoot(std::string_view name) {
    return {std::define_static_string(name)};
  }
  ...
  inline constexpr detail::NonemptySpec Nonempty{};
}
