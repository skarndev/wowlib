...
/** @file
    Flag-testing convenience for the wire formats' bit-mask enums.

    Wire structs store flag fields as plain integers — real files carry
...
{
  /** Whether flag bit @a flag is set in the raw wire value @a value.
      @tparam E    the scoped flag enum (deduced).
      @param value the wire field's raw integer value.
      @param flag  the named bit to test.
      @return true when every bit of @a flag is set in @a value. */
  template <typename E>
    requires std::is_scoped_enum_v<E>
  [[nodiscard]] constexpr bool has_flag(std::underlying_type_t<E> value, E flag)
  {
