  ...
  namespace detail {
    ...
    consteval bool has_validation_annotation(std::meta::info member) {
      for (std::meta::info spec :
           {
             ^^count_matches_spec,
             ^^count_multiple_of_spec,
             ^^count_exactly_spec,
             ^^indexes_spec,
             ^^indexes_optional_spec,
             ^^indexes_in_root_spec,
             ^^expected_value_spec,
             ^^nonempty_spec
           })
    ...
    void validate_index_elements(const Values& values,
    ...
    /** Report every non-sentinel element of @a values that is not a valid
        ...
        @param report       the report findings land in. */
    template <typename Values>
    void validate_optional_index_elements(const Values& values,
                                          std::size_t target_count,
                                          std::string_view member,
                                          std::string_view target,
                                          ValidationReport& report) {
    ...
    /** Apply one member's count_matches contract against its resolved sibling
        ...
        @param report       the report findings land in. */
    inline void validate_count_matches(std::size_t count,
                                       std::uint32_t scale,
                                       std::size_t target_count,
                                       std::string_view member,
                                       std::string_view target,
    ...
    void validate_members(const T& value, ValidationReport& report) {
      ...
      template for (constexpr auto m : members) {
        if constexpr (version_active<V, m>()) {
          ...
          if constexpr (constexpr auto ev = annotation<expected_value_spec, m>()
            ; ev.has_value()) {
          ...
          if constexpr (constexpr auto cm = annotation<count_matches_spec, m>();
            cm.has_value()) {
            ...
            if constexpr (SlotSequence<M>) {
              ...
              for (std::size_t slot = 0; slot < member.size(); ++slot)
                validate_count_matches(member[slot].size(), cm->scale,
                                       target.size(),
                                       std::format("{}[{}]", ident, slot),
                                       cm->view(), report);
            }
            else {
              validate_count_matches(member.size(), cm->scale, target.size(),
                                     ident, cm->view(),
                                     report);
          ...
          if constexpr (constexpr auto ix = annotation<indexes_spec, m>(); ix.
            has_value()) {
            static_assert(
              is_vector_v<M> && std::is_integral_v < typename M::value_type >,
            ...
            validate_index_elements(member, value.[:sibling:].size(), ident,
                                    ix->view(), report);
          }

          if constexpr (constexpr auto ixo = annotation<
              indexes_optional_spec, m>();
            ixo.has_value()) {
            static_assert(
              is_vector_v<M> && std::is_integral_v < typename M::value_type >,
            ...
            validate_optional_index_elements(member, value.[:sibling:].size(),
                                             ident, ixo->view(),
                                             report);
          }
