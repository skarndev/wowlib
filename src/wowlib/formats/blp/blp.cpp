...
namespace wowlib::formats::blp::detail {
  namespace {
    ...
    void place_block(const BlockPixels& block,
  ...
  void DxtCodec::decode_color_block(const std::uint8_t* block, BlockPixels& out, bool punch_through) noexcept {
  ...
  void DxtCodec::decode_interpolated_alpha(const std::uint8_t* block, std::span<std::uint8_t, 16> out) noexcept {
  ...
  BlockPixels DxtCodec::extract_block(std::span<const std::uint8_t> rgba,
                                      std::uint32_t width,
                                      std::uint32_t height,
                                      std::uint32_t bx,
                                      std::uint32_t by) noexcept {
    ...
    for (std::uint32_t by = 0; by < blocks_y; ++by)
      for (std::uint32_t bx = 0; bx < blocks_x; ++bx, at += stride) {
        ...
        case PixelFormat::Dxt1: {
          ...
          if (transparent) encode_punch_through_block(block, encoded.data());
          else
  ...
  std::uint8_t PaletteCodec::unpack_alpha(std::span<const std::byte> plane, std::size_t index) const noexcept {
    switch (alpha_depth_) {
    case 1: {
  ...
  void PaletteCodec::pack_alpha(std::uint8_t alpha, std::span<std::byte> plane, std::size_t index) const noexcept {
    switch (alpha_depth_) {
    case 1:
  ...
  Result<FileBuffer> PaletteCodec::encode(std::span<const std::uint8_t> rgba,
    ...
    for (std::size_t i = 0; i < texels; ++i) {
      ...
      if (found == memo.end()) found = memo.emplace(key, nearest_index(palette, r, g, b)).first;
  ...
  std::array<CImVector, blp_palette_size> PaletteCodec::build_palette(std::span<const std::uint8_t> rgba) const {
    ...
    while (boxes.size() < blp_palette_size) {
      ...
      if (pick == boxes.size() || pick_extent <= 0) break;
...
namespace wowlib::formats::blp {
  namespace {
    ...
    PixelFormat resolve_dxt_format(PixelFormat preferred, std::uint8_t alpha_depth) noexcept {
      ...
      case PixelFormat::Bc5:
        return preferred;
      default:
        return alpha_depth <= 1 ? PixelFormat::Dxt1 : alpha_depth == 4 ? PixelFormat::Dxt3 : PixelFormat::Dxt5;
      }
    }
  }
  ...
  Result<void> BLP::read(std::span<const std::byte> data) {
    if (data.size() < blp_header_bytes)
      return make_error(ErrorCode::ChunkTruncated,
                        std::format("BLP file is {} bytes; the header region is {}", data.size(), blp_header_bytes));
    ...
    if (header.color_encoding > std::to_underlying(ColorEncoding::BgraAlt))
      return make_error(ErrorCode::NotSupported, std::format("BLP colorEncoding {} is unknown", header.color_encoding));
    if (header.preferred_format > std::to_underlying(PixelFormat::Bc5) || header.preferred_format == 10)
      return make_error(ErrorCode::NotSupported,
  ...
  Result<FileBuffer> BLP::write() const {
    ...
    if (replay)
      for (std::size_t level = 0; level < blp_max_mips; ++level) {
        ...
        if (stored != present || (stored && mips[level].size() != stored_layout.sizes[level])) {
          replay = false;
          break;
        }
      }
    ...
    if (replay) {
      ...
      for (std::size_t level = 0; level < blp_max_mips; ++level)
        if (header.mip_offsets[level] != 0 && header.mip_sizes[level] != 0)
          std::memcpy(out.data() + header.mip_offsets[level], mips[level].data(), mips[level].size());
    ...
    if (mips.size() > blp_max_mips)
      return make_error(ErrorCode::InvalidEntityState,
                        std::format("a BLP addresses at most {} mip levels, {} stored", blp_max_mips, mips.size()));
    ...
    if (cursor > std::numeric_limits<std::uint32_t>::max())
      return make_error(ErrorCode::InvalidEntityState, "BLP mip payloads exceed the u32 offset space");
  ...
  Result<void> BLP::write(fs::FileSystem& fs, const FileKey& key) const {
    ...
    if (!resolved.path)
      return make_error(ErrorCode::PathNotResolvable, "saving a BLP needs a path for the file key");
    return write().and_then([&](FileBuffer data) {
      return fs.add_file(*resolved.path, data).transform([](auto&&) {});
    });
  }
  ...
  Result<Image> BLP::decode(std::uint32_t level) const {
    ...
    Result<std::vector<std::uint8_t>> pixels = [& ]() -> Result<std::vector<std::uint8_t>> {
      ...
      case ColorEncoding::Dxt:
        return detail::DxtCodec{resolve_dxt_format(preferred_format, alpha_depth)}.decode(src, w, h);
  ...
  Result<void> BLP::encode(const Image& image, const EncodeSettings& settings) {
    ...
    if (image.pixels.size() != std::size_t{image.width} * image.height * 4)
      return make_error(ErrorCode::InvalidEntityState, std::format("image holds {} pixel bytes; {}x{} RGBA needs {}",
    ...
    if (settings.alpha_depth != 0 && settings.alpha_depth != 1 && settings.alpha_depth != 4 && settings.alpha_depth !=
      8)
      return make_error(ErrorCode::InvalidEntityState,
                        std::format("alpha_depth must be 0, 1, 4 or 8, not {}", settings.alpha_depth));

    *this = BLP{};
    version = blp_version_1;
    color_encoding = settings.encoding == ColorEncoding::BgraAlt ? ColorEncoding::Bgra : settings.encoding;
    width = image.width;
    height = image.height;
    ...
    case ColorEncoding::Dxt:
      ...
      alpha_depth = dxt_format == PixelFormat::Dxt1
                      ? std::min<std::uint8_t>(settings.alpha_depth, 1)
  ...
  ValidationReport BLP::validate() const {
    ...
    if (width == 0 || height == 0)
      report.add_error("width", std::format("zero dimension ({}x{}): the client cannot size " "the texture", width,
                                            height));
    ...
    if (alpha_depth != 0 && alpha_depth != 1 && alpha_depth != 4 && alpha_depth != 8) {
      const std::string what = std::format("{} is not one of the 0/1/4/8 depths the client reads", alpha_depth);
      if (color_encoding == ColorEncoding::Palettized)
        report.add_error("alpha_depth", what + " and sizes this file's alpha plane");
      else report.add_warning("alpha_depth", what + "; ignored for this encoding");
    }
    if (mips.size() > blp_max_mips)
      report.add_error("mips", std::format("{} levels exceed the {} the header can address", mips.size(),
                                           blp_max_mips));
    ...
    for (std::size_t level = 0; level < mips.size() && !report.full(); ++level) {
      ...
      case ColorEncoding::Palettized:
        ...
        break;
      case ColorEncoding::Dxt:
        want = detail::DxtCodec{resolve_dxt_format(preferred_format, alpha_depth)}.encoded_size(w, h);
        break;
      case ColorEncoding::Bgra:
      case ColorEncoding::BgraAlt:
        want = pixels * 4;
      ...
      if (color_encoding == ColorEncoding::Palettized)
        ...
        report.add_warning(path, what + " — short levels decode as transparent black " "(a known client quirk)");
    }
    return report;
  }
}
