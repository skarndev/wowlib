...
namespace wowlib::formats::blp::detail {
  ...
  public:
    ...
    explicit DxtCodec(PixelFormat format) noexcept : _format{format} {}
    ...
    std::size_t blockBytes() const noexcept {
      return _format == PixelFormat::Dxt1 ? 8 : 16;
    }

    /** The exact payload size of a level per the block formula
        ceil(w/4) * ceil(h/4) * blockBytes().
  ...
  protected:
    /** Decode one BC1 color block (8 bytes) into 16 RGBA pixels.
        ...
        @param punchThrough whether the color0 <= color1 3-color mode maps
    ...
    static BlockPixels _extractBlock(std::span<const std::uint8_t> rgba,
                                     std::uint32_t width,
                                     std::uint32_t height,
                                     std::uint32_t bx,
  ...
  class PaletteCodec {
  public:
    /** @param alphaDepth the alpha plane's bits per pixel (0, 1, 4 or 8). */
    explicit PaletteCodec(std::uint8_t alphaDepth) noexcept : _alphaDepth{alphaDepth} {}
    ...
    std::size_t encodedSize(std::uint32_t width, std::uint32_t height) const noexcept {
      ...
      return texels + (texels * _alphaDepth + 7) / 8;
    }
    ...
    /** Encode RGBA8 pixels against an existing palette: nearest-entry indices
        plus the packed alpha plane, exactly encodedSize() bytes.
        @param rgba    width * height * 4 RGBA bytes.
  ...
  protected:
    ...
    static std::uint8_t _nearestIndex(std::span<const CImVector, BlpPaletteSize> palette,
                                      std::uint8_t r,
                                      std::uint8_t g,
