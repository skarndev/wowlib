...
namespace wowlib::formats::blp::detail
{
  ...
  public:
    ...
    [[nodiscard]]
    Result<FileBuffer> encode(std::span<const std::uint8_t> rgba, std::uint32_t width,
  ...
  protected:
    ...
    static void decode_color_block(const std::uint8_t* block, BlockPixels& out,
    ...
    static void decode_interpolated_alpha(const std::uint8_t* block,
    ...
    static void encode_punch_through_block(const BlockPixels& pixels,
    ...
    static BlockPixels extract_block(std::span<const std::uint8_t> rgba, std::uint32_t width,
  ...
  public:
    ...
    [[nodiscard]]
    std::array<CImVector, blp_palette_size> build_palette(
  ...
  protected:
    ...
    /** Pack one texel's alpha into the plane at this codec's bit depth.
        ...
        @param index the texel ordinal. */
    void pack_alpha(std::uint8_t alpha, std::span<std::byte> plane,
  ...
  public:
    ...
    [[nodiscard]]
    std::vector<std::uint8_t> downscale(std::span<const std::uint8_t> rgba, std::uint32_t width,
                                        std::uint32_t height) const;
  };
}
