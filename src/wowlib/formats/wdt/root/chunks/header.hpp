...
namespace wowlib::formats::wdt::root::chunks
{
  ...
  {
    uses_global_map_obj [[=welder::doc("A WMO-only map: one global map object (MWMO/MODF), "
                                       "no terrain tiles.")]] = 0x1,
    adt_has_mccv [[=welder::doc("ADTs carry MCCV vertex colors (WotLK+); with this set "
                                "every ADT of the map must have MCCV.")]] = 0x2,
    adt_has_big_alpha [[=welder::doc("Terrain shader 2 (_env): 4096-byte uncompressed MCAL "
    ...
    adt_has_doodadrefs_sorted_by_size_cat
      [[=welder::doc("ADT MCRF/MCRD doodad references are sorted by size "
                     "category.")]] = 0x8,
    adt_has_mclv [[=welder::doc("ADTs carry the second MCLV color layer (Cata+; deprecated "
                                "and forbidden in 8.x).")]] = 0x10,
    adt_has_upside_down_ground [[=welder::doc("Flip the ground upside down to create a "
                                              "ceiling (Cata+).")]] = 0x20,
    unk_0x40 [[=welder::doc("Unknown (MoP+); seen on Firelands2.wdt before Legion.")]] = 0x40,
    adt_has_height_texturing [[=welder::doc("Terrain shader 6: alpha maps are influenced by "
                                            "the _h textures and MTXP (MoP+); also 4096-byte "
                                            "uncompressed MCAL entries.")]] = 0x80,
    load_lod_adt [[=welder::doc("Load _lod.adt (Legion+); implicitly sets 0x8000.")]] = 0x100,
    has_maid [[=welder::doc("The WDT carries MAID: ADTs load by FileDataID instead of "
    ...
    unk_0x8000 [[=welder::doc("Affects _lod.adt rendering; implicitly set for the continent "
  ...
  namespace detail
  {
    ...
    ]] WOWLIB_EMPTY_BASES SMMapHeader<V> : WDTHeaderBase
    {
      ...
      [[=welder::doc("A legacy value the WotLK-era client reads but does not use "
    ...
    ]] WOWLIB_EMPTY_BASES SMMapHeader<V> : WDTHeaderBase
    {
      ...
      [[=welder::doc("FileDataID of the _fogs.wdt volumetric-fog satellite, or 0.")]]
      std::uint32_t fogs_fdid = 0;

      [[=welder::doc("FileDataID of the _mpv.wdt particulate-volume satellite, or 0.")]]
      std::uint32_t mpv_fdid = 0;

      [[=welder::doc("FileDataID of the _tex.wdt texture-list satellite, or 0.")]]
      std::uint32_t tex_fdid = 0;

      [[=welder::doc("FileDataID of the map's .wdl low-resolution heightmap, or 0.")]]
  ...
  template <ClientVersion V>
  using SMMapHeader =
    detail::SMMapHeader<canonical_version(V, wdt_header_pivots, wdt_versions)>;
  ...
  ]] AreaInfoFlags : std::uint32_t
  {
    ...
    all_water [[=welder::doc("The whole tile is \"fake water\" (Cataclysm+; pre-Cata "
