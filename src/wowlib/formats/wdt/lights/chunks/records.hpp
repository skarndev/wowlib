...
namespace wowlib::formats::wdt::lights::chunks {
  struct [[
      =welder::weld,
      =welder::doc(
        "One WoD point light (MPLT); Legion clients stop reading these in "
        "favor of MPL2.")
    ]] MapPointLightLegacy {
    ...
    [[=welder::doc(
      "Intensity and related parameters; exact meaning undocumented.")]]
  ...
  struct [[
    ...
    [[=welder::doc(
      "Unique light id within the WDT; increment for each new light.")]]
    std::uint32_t id = 0;
  ...
  struct [[
      =welder::weld,
      =welder::doc(
        "One Shadowlands point light (MPL3; replaces MPL2 in 9.0+ files).")
    ]] MapPointLight3 {
    [[=welder::doc("Unique light id within the WDT.")]]
    std::uint32_t id = 0;
  ...
  struct [[
      =welder::weld,
      =welder::doc(
        "One light texture animation (MLTA), referenced by mlta_index.")
    ]] LightAnimation {
