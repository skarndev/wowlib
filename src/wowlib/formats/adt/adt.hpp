...
namespace wowlib::formats::adt {
  ...
  namespace detail {
    ...
    struct ADTSplit {
      ...
      [[=welder::mark::exclude]] std::vector<std::byte> obj1_data;
      // raw _obj1.adt
      [[=welder::mark::exclude]] std::vector<std::byte> lod_data;
      // raw _lod.adt
  ...
  namespace detail {
    ...
    private:
      ...
      bool _write_engaged() const {
        ...
        if constexpr (id == "flying_bounds") return has_flag(header.flags, MapHeaderFlags::has_mfbo);
        else if constexpr (id == "textures") {
          if constexpr (requires { this->uses_texture_fdids; }) return !this->uses_texture_fdids;
        ...
        else if constexpr (formats::detail::annotation<formats::detail::optional_spec, m>().has_value()) return !this->
          [:m:].empty();
        else return true;
      }
  ...
  namespace detail {
    ...
    ValidationReport ADT<V>::validate() const {
      ...
      if (!chunks.empty() && chunks.size() != chunks_per_tile)
        report.add_error("chunks", std::format("count {} != the {} chunks of a tile", chunks.size(), chunks_per_tile));
      ...
      const std::size_t texture_count = [&] {
        if constexpr (requires { this->diffuse_texture_ids; })
          if (this->uses_texture_fdids) return this->diffuse_texture_ids.size();
        ...
        for (std::size_t j = 0; j < chunk.layers.size(); ++j)
          if (chunk.layers[j].texture_id >= texture_count)
            report.add_error(std::format("layers[{}]", j),
                             std::format("texture_id {} out of range: {} textures", chunk.layers[j].texture_id,
                                         texture_count));

        // the chunk's placement references index the TILE's tables
        formats::detail::validate_index_elements(chunk.doodad_refs, doodad_placements.size(), "doodad_refs",
                                                 "doodad_placements", report);
        formats::detail::validate_index_elements(chunk.object_refs, wmo_placements.size(), "object_refs",
                                                 "wmo_placements", report);
      ...
      const auto check_placements = [&](const auto& placements,
                                        auto fdid_flag,
                                        const auto& offsets,
                                        std::string_view what,
                                        std::string_view table) {
        for (std::size_t i = 0; i < placements.size() && !report.full(); ++i) {
          if (has_flag(placements[i].flags, fdid_flag)) continue;
          if (placements[i].name_id >= offsets.size())
            report.add_error(std::format("{}[{}]", what, i),
                             std::format("name_id {} out of range: {} holds {} entries", placements[i].name_id, table,
                                         offsets.size()));
        }
      };
      check_placements(doodad_placements, common::DoodadDefFlags::entry_is_fdid, model_name_offsets,
                       "doodad_placements", "model_name_offsets");
      check_placements(wmo_placements, common::MapObjDefFlags::entry_is_fdid, wmo_name_offsets, "wmo_placements",
                       "wmo_name_offsets");
      ...
      while (pos + 8 <= data.size()) {
        ...
        if (size > data.size() - pos - 8)
          return make_error(ErrorCode::ChunkTruncated,
                            std::format("ADT chunk {} at {:#x} overruns the file", fourcc_to_string(magic), pos));
        ...
        if (magic == four_cc("MCNK")) {
          if (chunks.size() < chunk_index + 1) chunks.resize(chunk_index + 1);
          if (auto r = chunks[chunk_index].read_from(payload, kind, alpha_format); !r) return r;
          ++chunk_index;
          continue;
        }
        ...
        template for (constexpr auto m : members) {
          if constexpr (constexpr auto spec = formats::detail::annotation<formats::detail::chunk_spec, m>(); spec.
            ...
            if (!matched && magic == spec->magic) {
              matched = true;
              outcome = formats::detail::read_value(this->[:m:], payload, magic, pos, spec->endian);
              ...
              if constexpr (std::meta::identifier_of(m) == "header") _normalize_mhdr();
                ...
                if constexpr (requires { this->uses_texture_fdids; }) this->uses_texture_fdids = true;
            }
          }
        }
    ...
    Result<FileBuffer> ADT<V>::write_file(FileKind kind, AlphaFormat alpha) const {
      ...
      std::array<std::pair<std::size_t, std::size_t>, 256> mcnk_loc{};
      ...
      for (const std::uint32_t want : chunk_order) {
        if (want == four_cc("MVER")) {
          _emit_chunk(out, want, [&] {
            const std::uint32_t v = adt_version_18;
            _put(out, &v, 4);
          });
          continue;
        }
          ...
          if constexpr (constexpr auto spec = formats::detail::annotation<formats::detail::chunk_spec, m>(); spec.
            ...
            if constexpr (constexpr auto route = formats::detail::annotation<in_file_spec, m>(); route.has_value()) {
              if (spec->magic == want && routes_to(route->file, kind) && _write_engaged<m>()) {
                ...
                const std::size_t at = _emit_chunk(out, want, [&] {
                  if (auto r = formats::detail::write_value(member, out); !r) err = r.error();
                });
                record(want, at);
                if (want == four_cc("MHDR")) mhdr_at = at;
              }
            }
          }
        }
      ...
      if (mono)
        for (std::size_t i = 0; i < 256; ++i) {
          const std::uint32_t entry[4]{
            static_cast<std::uint32_t>(mcnk_loc[i].first),
            static_cast<std::uint32_t>(mcnk_loc[i].second + 8),
            0,
            0
          };
    ...
    Result<void> ADT<V>::read(fs::FileSystem& fs, const FileKey& key, AlphaFormat alpha) {
      *this = ADT{};
      ...
      if constexpr (V < builds::Cata) {
        ...
        if (!resolved.path)
          return make_error(ErrorCode::PathNotResolvable, "loading a split ADT needs the root file path");
        ...
        chunks.assign(256, adt::MapChunk<V>{});
        if (auto r = parse_file(*root_data, FileKind::root); !r) return r;
        ...
        const auto load = [&](std::string_view suffix, FileKind fk) -> Result<void> {
          ...
          if (!data)
            return make_error(data.error().code, std::format("{} split file: {}", suffix, data.error().message));
        ...
        if constexpr (requires { this->obj1_data; }) {
          const auto keep = [&](std::string_view suffix, std::vector<std::byte>& into) {
            const FileKey k{sibling(suffix)};
            if (fs.exists(k))
              if (const auto data = fs.read_file(k)) into = *data;
          };
    ...
    Result<void> ADT<V>::write(fs::FileSystem& fs, const FileKey& key, AlphaFormat alpha) const {
      ...
      if constexpr (V < builds::Cata) {
        return write_file(FileKind::monolithic, alpha).and_then([&](FileBuffer data) {
        ...
        const auto store = [&](FileKind fk, std::string_view suffix) -> Result<void> {
          return write_file(fk, alpha).and_then([&](FileBuffer data) { return add(sibling(suffix), data); });
        };
        // the root file keeps the bare "{stem}.adt" name
        if (auto r = write_file(FileKind::root, alpha).and_then([&](FileBuffer data) {
          return add(*resolved.path, data);
        }); !r) return r;
        ...
        if constexpr (requires { this->obj1_data; }) {
          if (!this->obj1_data.empty())
            if (auto r = add(sibling("_obj1"), this->obj1_data); !r) return r;
          if (!this->lod_data.empty())
            if (auto r = add(sibling("_lod"), this->lod_data); !r) return r;
        }
        return {};
      }
    }
