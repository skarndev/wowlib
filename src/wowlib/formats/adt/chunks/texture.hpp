...
namespace wowlib::formats::adt::chunks
{
  ...
  ]] LayerFlags : std::uint32_t
  {
    ...
    use_alpha_map [[=welder::doc("This layer blends through an MCAL alpha map (set on every "
                                 "layer after the first).")]] = 0x100,
    alpha_map_compressed [[=welder::doc("The layer's MCAL alpha map is RLE-compressed on "
                                        "disk. wowlib decodes it either way; on write it "
                                        "re-compresses when this bit is set.")]] = 0x200,
    use_cube_map_reflection [[=welder::doc("Treat the ground layer as a skybox reflection "
                                           "(needs an MTXF entry).")]] = 0x400,
    unknown_0x800 [[=welder::doc("WoD?+ with 0x1000: apply the texture-effect texture "
                                 "scale.")]] = 0x800,
  ...
  {
    [[=welder::doc("The texture: an index into the tile's texture list (MTEX names, or "
                   "the MDID FileDataID table since 8.1).")]]
    std::uint32_t texture_id = 0;

    [[=welder::doc("Flags; LayerFlags bits (the low 6 bits are animation rotation/speed).")]]
    std::uint32_t flags = 0;

    [[=welder::mark::exclude]] std::uint32_t offset_in_mcal = 0;

    [[=welder::doc("The ground-effect link (a GroundEffectTexture foreign key), or -1 for "
                   "none (in the alpha clients, a u16 + padding).")]]
    std::int32_t effect_id = -1;

    [[nodiscard]]
    [[=welder::getter, =welder::doc("The animation direction in 45-degree steps (flag bits "
                                    "0x7).")]]
  ...
  ]] SMTextureParams
  {
    ...
    [[=welder::doc("Height scale: _h values map to [0, height_scale) as the blend height "
                   "(default 0.0).")]]
