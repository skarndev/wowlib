...
namespace wowlib::formats::adt
{
  ...
  {
    [[=welder::doc("The liquid type (a LiquidType foreign key: 1 ocean, 2 ocean-flat, "
                   "3 slime, 5/6 magma, …).")]]
    std::uint16_t liquid_type = 0;

    [[=welder::doc("The stored liquid_object_or_lvf field: a LiquidObject id (>= 42) or "
                   "a raw LiquidVertexFormat; vertex_format holds the resolved layout.")]]
    std::uint16_t liquid_object_or_lvf = 0;

    [[=welder::doc("The resolved vertex layout wowlib decoded (and will re-lay).")]]
    LiquidVertexFormat vertex_format = LiquidVertexFormat::height_depth;

    [[=welder::doc("Minimum surface height (the flat height when no heightmap).")]]
    float min_height = 0;
    ...
    [[=welder::doc("The per-tile exists mask, width x height bits row-major (LSB first); "
                   "empty means every tile renders."),
      =welder::mark::no_reassign]]
    std::vector<std::uint8_t> exists_bitmap;

    [[=welder::doc("Surface heights, (width+1) x (height+1) row-major; present for "
    ...
    [[=welder::doc("UV texture coordinates, (width+1) x (height+1) row-major; present "
  ...
  {
    [[=welder::doc("The 8x8 fishable bit mask (visibility); 0 when the cell omits "
                   "attributes.")]]
    std::uint64_t fishable = 0;

    [[=welder::doc("The 8x8 deep bit mask (fatigue water); 0 when the cell omits "
                   "attributes.")]]
    std::uint64_t deep = 0;

    [[=welder::doc("Whether this cell wrote an explicit attributes block (so an all-zero "
  ...
  ]] MCLQData
  {
    ...
    [[=welder::doc("The 8x8 = 64 tile flag bytes (bits: liquid type, don't-render, "
                   "fatigue)."),
      =welder::mark::no_reassign]]
  ...
  namespace detail
  {
    ...
    inline LiquidVertexFormat resolve_vertex_format(std::uint16_t stored,
                                                    std::size_t total_bytes, std::size_t n)
    {
      if (n != 0 && total_bytes % n == 0)
        ...
        case 9: return LiquidVertexFormat::height_uv_depth; // float + 2 u16 + byte
        default: break;
        }
      // no vertex block (ocean): trust the stored LVF when small, else flat depth
      if (stored <= 3)
        return static_cast<LiquidVertexFormat>(stored);
  ...
  inline Result<void> MH2OData::read(std::span<const std::byte> payload)
  {
    ...
    if (payload.size() < header_bytes)
      return make_error(ErrorCode::ChunkTruncated,
                        std::format("MH2O needs {} header bytes, got {}", header_bytes,
                                    payload.size()));
    ...
    for (std::size_t c = 0; c < 256; ++c)
    {
      ...
      if (ofs_attr)
      {
        if (ofs_attr + 16 > payload.size())
          return make_error(ErrorCode::ChunkTruncated,
                            std::format("MH2O attributes of cell {} overrun", c));
      ...
      for (std::uint32_t k = 0; k < count; ++k)
      {
        ...
        if (ofs_exists)
        {
          ...
          if (ofs_exists + bytes > payload.size())
            return make_error(ErrorCode::ChunkTruncated,
                              std::format("MH2O exists mask of cell {} overruns", c));
          inst.exists_bitmap.assign(
            reinterpret_cast<const std::uint8_t*>(payload.data() + ofs_exists),
        ...
        if (ofs_verts)
        {
          ...
          inst.vertex_format =
            detail::resolve_vertex_format(inst.liquid_object_or_lvf, total, n);
          std::size_t p = ofs_verts;
          const auto has_height = inst.vertex_format == LiquidVertexFormat::height_depth
                                  || inst.vertex_format == LiquidVertexFormat::height_uv
                                  || inst.vertex_format == LiquidVertexFormat::height_uv_depth;
          const auto has_uv = inst.vertex_format == LiquidVertexFormat::height_uv
                              || inst.vertex_format == LiquidVertexFormat::height_uv_depth;
          const auto has_depth = inst.vertex_format == LiquidVertexFormat::height_depth
                                 || inst.vertex_format == LiquidVertexFormat::depth_only
                                 || inst.vertex_format == LiquidVertexFormat::height_uv_depth;
          if (has_height)
          {
          ...
          if (has_depth)
          {
            inst.depthmap.assign(
              reinterpret_cast<const std::uint8_t*>(payload.data() + p),
        ...
        {
          inst.vertex_format =
            detail::resolve_vertex_format(inst.liquid_object_or_lvf, 0, n);
        }
  ...
  inline Result<void> MH2OData::write(FileBuffer& out) const
  {
    ...
    struct InstLoc { std::size_t header_at; std::size_t exists_at; std::size_t verts_at; };
    ...
    for (std::size_t c = 0; c < n_cells; ++c)
    {
      ...
      for (std::size_t k = 0; k < cell.instances.size(); ++k)
      {
        ...
        const bool any_verts =
          !inst.heightmap.empty() || !inst.depthmap.empty() || !inst.uvmap.empty();
        if (any_verts)
        {
  ...
  inline Result<void> MCLQData::write(FileBuffer& out) const
  {
    ...
    put(&n_flows, 4);
    SWFlowv pair[2]{};
    for (std::size_t i = 0; i < flows.size() && i < 2; ++i)
      pair[i] = flows[i];
    put(pair, 80);
    return {};
  }
