...
namespace wowlib::formats::m2
{
  ...
  public:
      ...
      : stem_{model_path}
    {
      for (std::string_view ext :
           {".m2", ".M2", ".mdx", ".MDX", ".mdl", ".MDL", ".skel", ".SKEL"})
  ...
  public:
    ...
    static std::uint32_t afid_lookup(std::span<const chunked::record::AnimFileEntry> entries,
                                     SequenceKey key)
    {
      for (const chunked::record::AnimFileEntry& e : entries)
        if (e.anim_id == key.id && e.sub_anim_id == key.variation)
          return e.file_id;
      return 0;
    }
    ...
    auto sequence_base(const std::vector<Sequence>& sequences,
                       std::span<const std::byte> inline_base, std::uint32_t target)
    {
      return [this, &sequences, inline_base,
              target](std::size_t i) -> std::span<const std::byte> {
        if (i >= sequences.size())
          return inline_base;
  ...
  public:
    ...
    void append_chunk_to(FileBuffer& out, std::uint32_t fourcc, SequenceKey key) const
    {
      ...
      append_chunk(out, fourcc,
                   it == bufs_.end() ? std::span<const std::byte>{}
    ...
    static std::set<SequenceKey> merged_keys(std::initializer_list<const AnimBuffers*> sets)
    {
      ...
      for (const AnimBuffers* set : sets)
        for (const auto& [key, buf] : set->bufs_)
          keys.insert(key);
      return keys;
    }
