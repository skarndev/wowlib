...
namespace wowlib::formats::m2 {
  ...
  namespace detail {
    ...
    private:
      ...
      static Result<void> _read_skin_into(fs::FileSystem& fs, const FileKey& key, std::string_view what, auto& out)
      ...
      Result<void> _load_skeleton(fs::FileSystem& fs) requires (V >= m2_chunked_container);
      ...
      /** Decode the MD20 body out of the MD21 image with FileDataID-based .anim
          ...
          @return nothing, or the first error. */
      Result<void> _read_chunked_body(fs::FileSystem& fs, const FileKey& key, bool has_skel) requires (V >=
        m2_chunked_container);
      ...
      /** Load the LOD views from SFID: the first num_skin_profiles entries are
          ...
          @param fs the filesystem gateway.
          @return nothing, or the first error. */
      Result<void> _read_chunked_skins(fs::FileSystem& fs) requires (V >= m2_chunked_container);
      ...
      Result<void> _read_bone_files(fs::FileSystem& fs, bool has_skel) requires (V >= m2_chunked_container);
      ...
      /** Bake the referenced .phys file into `phys` verbatim (inline PFDC stays
          ...
          @param fs the filesystem gateway. */
      void _read_physics(fs::FileSystem& fs) requires (V >= m2_chunked_container);
      ...
      Result<FileBuffer> _write_body_image(AnimBuffers& afm2_bufs, const auto& sequences) const requires (V >=
        m2_per_sequence_timelines);
      ...
      /** Non-skel .anim write: one file per external sequence (AFM2-wrapped when
          ...
          @return nothing, or the first error. */
      Result<void> _write_plain_anims(fs::FileSystem& fs,
                                      const SatellitePaths& paths,
                                      AnimBuffers& afm2_bufs,
                                      auto& stream) const requires (V >= m2_chunked_container);
      ...
      /** Skel-based satellite write: re-encode the skeleton's bone/attachment
          ...
          @return nothing, or the first error. */
      Result<void> _write_skeleton_satellites(fs::FileSystem& fs,
                                              const SatellitePaths& paths,
                                              AnimBuffers& afm2_bufs,
      ...
      /** Write the .skin views and LOD bands, recording their FileDataIDs into
          ...
          @return nothing, or the first error. */
      Result<void> _write_chunked_skins(fs::FileSystem& fs, const SatellitePaths& paths, auto& stream) const requires (V
        >= m2_chunked_container);
      ...
      /** Bake the .phys file (when present) and record its FileDataID into
          ...
          @return nothing, or the first error. */
      Result<void> _write_chunked_phys(fs::FileSystem& fs, const SatellitePaths& paths, auto& stream) const requires (V
        >= m2_chunked_container);
...
namespace wowlib::formats::m2 {
  ...
  Result<void> detail::M2<V>::_check_header(std::uint32_t magic, std::uint32_t format_version) {
    ...
    if (format_version < range.first || format_version > range.second)
      return make_error(ErrorCode::FormatVersionMismatch,
                        std::format("MD20 version {} is outside the requested client's " "era [{}, {}]", format_version,
                                    range.first, range.second));
    return {};
  }

  template <ClientVersion V>
  Result<void> detail::M2<V>::_read_skin_into(fs::FileSystem& fs, const FileKey& key, std::string_view what, auto& out)
    requires (V >= m2_per_sequence_timelines) {
    ...
    if (auto r = skin.read(std::span<const std::byte>{*bytes}); !r)
      return make_error(r.error().code, std::format("{}: {}", what, r.error().message), r.error().native_error);
  ...
  Result<void> detail::M2<V>::_read_monolithic(fs::FileSystem& fs, const FileKey& key, std::span<const std::byte> main)
    ...
    if (auto r = _check_header(this->root.magic, this->root.format_version); !r) return r;

    this->skins.reserve(this->root.num_skin_profiles);
    for (std::uint32_t i = 0; i < this->root.num_skin_profiles; ++i)
      if (auto r = _read_skin_into(fs, FileKey{paths.skin(i)}, std::format("skin {}", i), this->skins); !r) return r;
    return {};
  }
  ...
  Result<void> detail::M2<V>::_read_chunked(fs::FileSystem& fs, const FileKey& key, std::span<const std::byte> main)
    ...
    if (auto r = _read_chunked_body(fs, key, has_skel); !r) return r;
    ...
    return _read_chunked_skins(fs).and_then([&] { return _read_bone_files(fs, has_skel); }).transform([&] {
      _read_physics(fs);
    });
  }

  template <ClientVersion V>
  Result<void> detail::M2<V>::_load_skeleton(fs::FileSystem& fs) requires (V >= m2_chunked_container) {
    if (auto r = this->skel.read(fs, FileKey{FileDataID{this->chunks.skeleton_fdid.front()}}); !r)
      return make_error(r.error().code, std::format(".skel: {}", r.error().message), r.error().native_error);
    return {};
  }

  template <ClientVersion V>
  Result<void> detail::M2<V>::_read_chunked_body(fs::FileSystem& fs, const FileKey& key, bool has_skel) requires (V >=
    m2_chunked_container) {
    ...
    const auto* sequences = has_skel ? &this->skel.sequence_block.sequences : &this->root.sequences;
  ...
  template <ClientVersion V>
  Result<void> detail::M2<V>::_read_chunked_skins(fs::FileSystem& fs) requires (V >= m2_chunked_container) {
    ...
    if (this->chunks.skin_fdids.size() < this->root.num_skin_profiles)
      return make_error(ErrorCode::InvalidEntityState, std::format("SFID holds {} entries, the body declares {} views",
                                                                   this->chunks.skin_fdids.size(),
                                                                   this->root.num_skin_profiles));
    for (const auto [i, fdid] : std::views::enumerate(this->chunks.skin_fdids) | std::views::take(
           this->root.num_skin_profiles))
      if (auto r = _read_skin_into(fs, FileKey{FileDataID{fdid}}, std::format("skin {}", i), this->skins); !r) return r;
    for (const auto [i, fdid] : std::views::enumerate(this->chunks.skin_fdids) | std::views::drop(
           this->root.num_skin_profiles))
      if (fdid != 0)
        if (auto r = _read_skin_into(fs, FileKey{FileDataID{fdid}}, std::format("lod skin {}", i), this->lod_skins); !r)
          return r;
    return {};
  }

  template <ClientVersion V>
  Result<void> detail::M2<V>::_read_bone_files(fs::FileSystem& fs, bool has_skel) requires (V >= m2_chunked_container) {
    ...
    for (const auto [i, fdid] : std::views::enumerate(bfids)) {
      ...
      if (!bytes)
        return make_error(bytes.error().code, std::format(".bone {}: {}", i, bytes.error().message),
                          bytes.error().native_error);
      BoneFile bone;
      if (auto r = bone.read(std::span<const std::byte>{*bytes}); !r)
        return make_error(r.error().code, std::format(".bone {}: {}", i, r.error().message), r.error().native_error);
  ...
  template <ClientVersion V>
  void detail::M2<V>::_read_physics(fs::FileSystem& fs) requires (V >= m2_chunked_container) {
    if (!this->chunks.phys_fdid.empty() && this->chunks.phys_fdid.front() != 0)
      if (auto bytes = fs.read_file(FileKey{FileDataID{this->chunks.phys_fdid.front()}})) this->phys.bytes =
        std::move(*bytes);
  }
  ...
  Result<void> detail::M2<V>::read(fs::FileSystem& fs, const FileKey& key) {
    ...
    root = {};
    if constexpr (V >= m2_per_sequence_timelines) this->skins.clear();
    ...
    if constexpr (V < m2_per_sequence_timelines) {
      ...
      if (lead == md20_magic) {
        ...
        if constexpr (V < m2_chunked_only) return _read_monolithic(fs, key, std::span<const std::byte>{*main});
        else
          return make_error(ErrorCode::FormatVersionMismatch,
  ...
  Result<void> detail::M2<V>::write(fs::FileSystem& fs, const FileKey& key) const {
    ...
    else if constexpr (V < m2_chunked_container) return _write_monolithic(fs, *resolved.path, paths);
    else return _write_chunked(fs, *resolved.path, paths);
  }

  template <ClientVersion V>
  Result<FileBuffer> detail::M2<V>::_write_body_image(AnimBuffers& afm2_bufs, const auto& sequences) const requires (V
    >= m2_per_sequence_timelines) {
  ...
  Result<void> detail::M2<V>::_write_monolithic(fs::FileSystem& fs,
    ...
    m2_per_sequence_timelines && V < m2_chunked_container) {
    AnimBuffers afm2_bufs;
    ...
    if (auto r = fs.add_file(path, *bytes); !r) return std::unexpected{r.error()};
    ...
    for (const auto [i, skin] : std::views::enumerate(this->skins)) {
      ...
      if (!skin_bytes) return std::unexpected{skin_bytes.error()};
      if (auto r = fs.add_file(paths.skin(static_cast<std::uint32_t>(i)), *skin_bytes); !r)
        return make_error(r.error().code, std::format("skin {}: {}", i, r.error().message), r.error().native_error);
    }
    return {};
  }
  ...
  Result<void> detail::M2<V>::_write_chunked(fs::FileSystem& fs,
                                             const std::string& path,
                                             const SatellitePaths& paths) const requires (V >= m2_chunked_container) {
    ...
    AnimBuffers afm2_bufs;
    auto bytes = has_skel
                   ? _write_body_image(afm2_bufs, this->skel.sequence_block.sequences)
    ...
    if (!bone_fdids) return std::unexpected{bone_fdids.error()};
    ...
    if (!has_skel) {
      if (auto r = _write_plain_anims(fs, paths, afm2_bufs, stream); !r) return r;
      stream.bone_fdids = *bone_fdids;
    }
    else if (auto r = _write_skeleton_satellites(fs, paths, afm2_bufs, this->skel.sequence_block.sequences, *bone_fdids,
                                                 stream); !r) return r;
    ...
    // first failure.
    return _write_chunked_skins(fs, paths, stream).and_then([&] { return _write_chunked_phys(fs, paths, stream); }).
                                                   and_then([&] { return stream.write(); }).and_then(
                                                     [&](const FileBuffer& stream_bytes) {
                                                       return fs.add_file(path, stream_bytes).transform(
                                                         [](FileDataID) {});
                                                     });
  }
  ...
  detail::M2<V>::_write_bone_files(fs::FileSystem& fs, const SatellitePaths& paths) const requires (V >=
    ...
    for (const auto [i, bone] : std::views::enumerate(this->bone_files)) {
      ...
      if (!bone_bytes) return std::unexpected{bone_bytes.error()};
  ...
  template <ClientVersion V>
  Result<void> detail::M2<V>::_write_plain_anims(fs::FileSystem& fs,
                                                 const SatellitePaths& paths,
                                                 AnimBuffers& afm2_bufs,
                                                 auto& stream) const requires (V >= m2_chunked_container) {
    ...
    for (const auto& [seq, buf] : afm2_bufs.entries()) {
      FileBuffer file;
      if (has_flag(root.global_flags, GlobalFlags::ChunkedAnimFiles)) afm2_bufs.append_chunk_to(
        file, AnimCache::afm2_magic, seq);
      else file = buf;
  ...
  template <ClientVersion V>
  Result<void> detail::M2<V>::_write_skeleton_satellites(fs::FileSystem& fs,
                                                         const SatellitePaths& paths,
                                                         AnimBuffers& afm2_bufs,
                                                         const auto& sequences,
                                                         const std::vector<std::uint32_t>& bone_fdids,
    ...
    if (!skel_bytes) return std::unexpected{skel_bytes.error()};
  ...
  template <ClientVersion V>
  Result<void> detail::M2<V>::_write_chunked_skins(fs::FileSystem& fs, const SatellitePaths& paths, auto& stream) const
    requires (V >= m2_chunked_container) {
    ...
    for (const auto [i, skin] : std::views::enumerate(this->lod_skins)) {
      ...
      if (!r)
        return make_error(r.error().code, std::format("lod skin {}: {}", i, r.error().message), r.error().native_error);
      stream.skin_fdids.push_back(r->value);
    }
    return {};
  }
  ...
  template <ClientVersion V>
  Result<void> detail::M2<V>::_write_chunked_phys(fs::FileSystem& fs, const SatellitePaths& paths, auto& stream) const
    requires (V >= m2_chunked_container) {
    stream.phys_fdid.clear();
    if (!this->phys.bytes.empty()) {
      const auto r = fs.add_file(paths.phys(), this->phys.bytes);
      if (!r)
        return make_error(r.error().code, std::format(".phys: {}", r.error().message), r.error().native_error);
  ...
  namespace detail {
    ...
    void validate_profile_against_root(const Profile& profile, const Root& root, ValidationReport& report) {
      formats::detail::validate_index_elements(profile.vertices, root.vertices.size(), "vertices", "the model vertices",
                                               report);

      for (std::size_t i = 0; i < profile.submeshes.size() && !report.full(); ++i) {
        const auto& submesh = profile.submeshes[i];
        if (std::size_t{submesh.bone_combo_index} + submesh.bone_count > root.bone_lookup_table.size())
          report.add_error(std::format("submeshes[{}]", i),
                           std::format("bone-lookup range [{}, {}) overruns the {} entries", submesh.bone_combo_index,
                                       submesh.bone_combo_index + submesh.bone_count, root.bone_lookup_table.size()));
      }
      ...
      for (std::size_t i = 0; i < profile.batches.size() && !report.full(); ++i) {
        ...
        if (batch.material_index >= root.materials.size())
          report.add_error(path, std::format("material_index {} out of range: {} materials", batch.material_index,
                                             root.materials.size()));
        if (!formats::detail::is_no_index(batch.color_index) && batch.color_index >= root.colors.size())
          report.add_error(path, std::format("color_index {} out of range: {} colors", batch.color_index,
                                             root.colors.size()));
        ...
        if (std::size_t{batch.texture_combo_index} + batch.texture_count > root.texture_lookup_table.size())
          report.add_error(path, std::format("texture-lookup range [{}, {}) overruns the {} entries",
                                             batch.texture_combo_index, batch.texture_combo_index + batch.texture_count,
                                             root.texture_lookup_table.size()));
        const auto lookup_start = [&](std::uint16_t first, const auto& table, std::string_view what) {
          if (!table.empty() && first >= table.size())
            report.add_error(path, std::format("{} start {} out of range: {} entries", what, first, table.size()));
        };
        lookup_start(batch.texture_weight_combo_index, root.transparency_lookup_table, "transparency-lookup");
  ...
  ValidationReport detail::M2<V>::validate() const {
    ...
    // .skel, so only the assembly can resolve this.
    {
      ...
      if constexpr (requires { this->skel.bone_block.bones; })
        if (root.bones.empty() && !this->skel.bone_block.bones.empty()) bones = &this->skel.bone_block.bones;
      formats::detail::validate_optional_index_elements(root.bone_lookup_table, bones->size(), "root.bone_lookup_table",
                                                        "the model bones", report);
      formats::detail::validate_optional_index_elements(root.key_bone_lookup, bones->size(), "root.key_bone_lookup",
                                                        "the model bones", report);
    }
