...
The typed wire structs of an ADT file — the per-cell **MCNK header**
...
the **flying bounds** (`MFBOPlanes`) and the sound emitter. Wire integer fields
show their on-disk width. The derived offset/size/count fields the serializer
stamps are not exposed.
