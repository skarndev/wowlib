...
## Validation contracts page (2026-08-10)

`content/guide/validation.md` explains why write() does not validate, how to
read a report, and the error-vs-warning policy; its `<!-- validation-contracts -->`
marker is filled by `validation_reference_impl.py` (+ the usual reload shim
`validation_reference.py`, registered in mkdocs.yml hooks and build.py's watch
list).

- **Generated by SCANNING the C++ sources** for the validation annotations
  (count_matches/count_exactly/count_multiple_of/indexes/indexes_optional/
  indexes_in_root/expected_value/nonempty), so the table cannot drift: adding a
  contract adds a row. Rows render the CONTRACT ("one per `vertices` entry"),
  never the annotation spelling.
- **Version-range traits are mapped to their entity.** The scanner parses each
  entity's `slot<V, since, Trait>` base list and rewrites `GroupBody90` ->
  `WMOGroupBody`, `Root73` -> `WMORoot`: welder flattens those members onto the
  binding, so the trait names are invisible to users and must be invisible here.
- **Hand-written checks are listed too**, detected from `validate_extra` AND from
  a hand-written `ValidationReport X::validate()` (that second form is the only
  way BLP and db::Table show up — neither has annotations). The CRTP mixins
  (ChunkedFile, M2OffsetBlock) are excluded: their validate() is the engine
  entry, not a format's contracts.
- **`check()` fails the docs build** when a format known to carry contracts
  yields none — a renamed annotation or restyled `[[ … ]]` block would otherwise
  silently empty the page.
- GOTCHA: `_type_name` scans up to 40 lines past `struct` for the name, because
  the welded types put a multi-line `[[ … ]]` doc block in between — WDL's is 13
  lines and a 12-line lookahead silently attributed its hook to the last trait
  (`WdlTww`).
- PRE-EXISTING (not from this page): Doxygen warns "end of comment block while
  expecting command </u>" on offset_block.hpp — the M2 `_underscore` method
  convention (commit e9dd538) reads as markdown underline to Doxygen.
