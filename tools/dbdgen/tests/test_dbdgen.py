...
"""
    ...
    def test_emit_shard_shape(self):
        shard = emit_shard(3, [("Widget", self._ranges())])
        self.assertIn("// Generated by dbdgen — db binding shard 3.", shard)
        self.assertIn('#include "db_shard.hpp"', shard)
        self.assertIn("#include <wowlib/db/tables/widget.hpp>", shard)
        # Per-range concrete aliases (table + record), named by range_suffix.
        self.assertIn("using WidgetVanilla = Widget<versions::vanilla>;", shard)
        self.assertIn("using WidgetRecordWotlk = WidgetRecord<versions::wotlk>;",
                      shard)
        # A distinct non-inline register function that weld_type's each concrete
        # type (weld_namespace would ODR-merge across shards).
        self.assertIn("void register_shard_3(", shard)
        self.assertIn("W::weld_type<rb::Widget>(rowbase);", shard)
        self.assertIn("W::weld_type<t::Widget_>(tables);", shard)
        self.assertIn('W::weld_type<t::WidgetVanilla>(tables, "WidgetVanilla");',
                      shard)
    ...
    def test_emit_shard_registry_shape(self):
        ...
        self.assertIn("void register_shard_0(", registry)
        self.assertIn("void register_shard_1(", registry)
        self.assertNotIn("register_shard_2(", registry)
